...
**Must-have.** Pi already supports 25+ providers natively. v3 does not write per-provider TypeScript shims at M1. It ships `runtime/providers/role-resolver.ts` consuming Pi's catalogue plus `runtime/providers/quirks.json` for overrides (Anthropic `thinkingLevelMap`, OpenAI `compat.maxTokensField`, etc.). (TDD2 §7.5, §13.1, ADR-003) — *supersedes the older TDD.md claim of "Anthropic + OpenAI shims at M1."*
