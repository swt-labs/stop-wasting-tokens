...
require_grep "debug calls selected-todo start helper" 'debug-start-selected-todo\.sh" \.swt-planning <N>' "$DEBUG_CMD"
...
require_absent "debug no longer runs selected-todo pickup directly" 'todo-lifecycle\.sh" pickup /swt:debug' "$DEBUG_CMD"
...
require_absent "research does not claim todos automatically" 'todo-lifecycle\.sh" pickup /swt:research' "$RESEARCH_CMD"
