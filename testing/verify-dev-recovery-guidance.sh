...
# verify-dev-recovery-guidance.sh — Verify swt-dev.md has deterministic recovery rules
#
...
if grep -qi 'read.*before.*edit\|read.*before.*writ\|file has not been read\|read the target file' "$DEV_FILE"; then
  pass "swt-dev.md: mentions read-before-edit recovery"
else
  fail "swt-dev.md: missing read-before-edit recovery guidance"
fi

# 2. Retry-once-then-escalate pattern
if grep -qi 'retry.*once\|retry the.*edit\|retry the.*write\|one retry' "$DEV_FILE"; then
  pass "swt-dev.md: mentions retry-once pattern for tool precondition errors"
else
  fail "swt-dev.md: missing retry-once-then-escalate for tool precondition errors"
fi
...
if grep -qi 'contradicts\|contradiction.*blocker\|contradictory.*validation\|contradictory.*result' "$DEV_FILE"; then
  pass "swt-dev.md: mentions validation contradiction as blocker"
else
  fail "swt-dev.md: missing validation-contradiction-as-blocker guidance"
fi
...
if grep -qi 'empty.*result.*blocker\|empty.*response.*blocker\|empty.*result.*escalat\|empty.*validation.*stop\|empty.*result.*not.*success\|empty results are not success' "$DEV_FILE"; then
  pass "swt-dev.md: mentions empty validation results as blocker condition"
else
  fail "swt-dev.md: missing empty-result-as-blocker guidance"
fi
...
if grep -qi 'reread.*loop\|no.progress.*loop\|rereading.*same.*file\|read.*loop.*circuit\|no forward progress' "$DEV_FILE"; then
  pass "swt-dev.md: mentions reread/no-progress loop detection"
else
  fail "swt-dev.md: missing reread/no-progress loop detection guidance"
fi
...
if grep -qi 'no.progress.*escalat\|reread.*escalat\|no.progress.*blocker\|reread.*blocker\|two consecutive.*no.progress' "$DEV_FILE"; then
  pass "swt-dev.md: reread loops trigger escalation"
else
  fail "swt-dev.md: reread loops not linked to escalation/blocker"
fi
