...
promote_todos() {
  ...
  local planning_dir="${VBW_PLANNING_DIR:-}"
  if [ -z "$planning_dir" ]; then
    # PHASE_DIR is like .swt-planning/phases/03-slug/, so go up two levels
    planning_dir=$(cd "$PHASE_DIR/../.." && pwd)
  fi
  ...
  local detail_script=""
  local session_key="${SWT_SESSION_ID:-default}"
  local link="/tmp/.swt-install-root-link-${session_key}"
