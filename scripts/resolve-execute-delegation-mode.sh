...
PHASE_DIR=""
CONFIG_PATH=".swt-planning/config.json"
EXEC_STATE_PATH=".swt-planning/.execution-state.json"
ROUTE_MAP_PATH=""
SEGMENTS_MODE=false
...
while IFS=$'\t' read -r raw_id raw_status; do
  ...
  plan_path=""
  resolve_err_file=$(mktemp "${TMPDIR:-/tmp}/swt-resolve-plan.XXXXXX")
...
for wave in $(jq -cr '.[]' <<< "$waves_json"); do
  ...
  if [ "$delegate_wave_count" -gt 0 ]; then
    ...
    if [ "$seg_mode" = "team" ]; then
      ...
      team_name="swt-phase-${phase_id}"
      if [ "$team_segment_index" -gt 1 ]; then
        team_name="swt-phase-${phase_id}-segment-${team_segment_index}"
      fi
    fi
    segments_json=$(jq -c \
      --arg route "delegate" \
