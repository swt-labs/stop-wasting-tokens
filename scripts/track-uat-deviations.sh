...
record_from_uat() {
  ...
  tmp_records=$(mktemp "${TMPDIR:-/tmp}/swt-accepted-deviations.XXXXXX")
  tmp_registry=$(mktemp "${TMPDIR:-/tmp}/swt-accepted-registry.XXXXXX")
...
planning_root_for_phase() {
  ...
  while [ "$dir" != "/" ] && [ -n "$dir" ]; do
    if [ "$(basename "$dir")" = ".swt-planning" ]; then
      printf '%s\n' "$dir"
      return 0
    fi
    dir=$(dirname "$dir")
