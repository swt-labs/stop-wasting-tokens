#!/bin/bash
set -u
# SessionStart: SWT project state detection, update checks, cache maintenance (exit 0)

# --- Dependency check ---
if ! command -v jq &>/dev/null; then
  echo '{"hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":"SWT: jq not found. Install: brew install jq (macOS) / apt install jq (Linux). All 17 SWT quality gates are disabled until jq is installed -- no commit validation, no security filtering, no file guarding."}}'
  exit 0
fi
...
. "$SCRIPT_DIR/lib/swt-config-root.sh"
# shellcheck source=lib/swt-cache-key.sh
. "$SCRIPT_DIR/lib/swt-cache-key.sh"
find_vbw_root
...
# session_id. Since SWT_SESSION_ID was removed from the env (upstream
...
if [ -n "$_VBW_SESSION_ID" ] && [ -n "${CLAUDE_ENV_FILE:-}" ]; then
  _EXISTING_SID=$(grep '^export SWT_SESSION_ID=' "$CLAUDE_ENV_FILE" 2>/dev/null | head -1 | sed 's/^export SWT_SESSION_ID=//; s/^"//; s/"$//' || true)
  if [ -z "$_EXISTING_SID" ]; then
    printf 'export SWT_SESSION_ID="%s"\n' "$_VBW_SESSION_ID" >> "$CLAUDE_ENV_FILE"
    ...
    grep -v '^export SWT_SESSION_ID=' "$CLAUDE_ENV_FILE" > "$_tmp_env" 2>/dev/null || true
...
# Auto-migrate config if .swt-planning exists.
...
# Old SWT versions wrote a duplicate isolation guard to .claude/CLAUDE.md.
...
if [ "$_bf_bad_summary_count" -gt 0 ]; then
  UPDATE_MSG="${UPDATE_MSG} BROWNFIELD: ${_bf_bad_summary_count} SUMMARY.md file(s) lack valid completion status (missing 'status: complete' in YAML frontmatter). These plans will not be counted as complete. Fix by adding frontmatter or re-execute with swt cook."
fi

# --- First-run welcome (DXP-03) ---
VBW_MARKER="$CLAUDE_DIR/.swt-welcomed"
WELCOME_MSG=""
if [ ! -f "$VBW_MARKER" ]; then
  mkdir -p "$CLAUDE_DIR" 2>/dev/null
  touch "$VBW_MARKER" 2>/dev/null
  WELCOME_MSG="FIRST RUN -- Display this welcome to the user verbatim: Welcome to SWT -- Vibe Better with Claude Code. You're not an engineer anymore. You're a prompt jockey with commit access. At least do it properly. Quick start: swt cook -- describe your project and SWT handles the rest. Type swt help for the full story. --- "
fi

# --- Update check (once per day, fail-silent) ---

_VBW_UPDATE_CACHE_KEY=$(vbw_hash_path "$SCRIPT_DIR")
CACHE="/tmp/swt-update-check-$(id -u)-${_VBW_UPDATE_CACHE_KEY}"
NOW=$(date +%s)
...
if [ ! -f "$CACHE" ] || [ $((NOW - MT)) -gt 86400 ]; then
  ...
  if [ -n "$REMOTE_VER" ] && [ "$REMOTE_VER" != "0.0.0" ] && [ "$REMOTE_VER" != "$LOCAL_VER" ]; then
    UPDATE_MSG="${UPDATE_MSG} UPDATE AVAILABLE: v${LOCAL_VER} -> v${REMOTE_VER}. Run swt update to upgrade."
  fi
else
  # Read cached result
  ...
  if [ -n "${REMOTE_VER:-}" ] && [ "${REMOTE_VER:-}" != "0.0.0" ] && [ "${REMOTE_VER:-}" != "${LOCAL_VER:-}" ]; then
    UPDATE_MSG="${UPDATE_MSG} UPDATE AVAILABLE: v${LOCAL_VER:-0.0.0} -> v${REMOTE_VER:-0.0.0}. Run swt update to upgrade."
  fi
fi
...
if [ -f "$SETTINGS_FILE" ]; then
  ...
  if echo "$SL_CMD" | grep -q 'for f in' && echo "$SL_CMD" | grep -q 'swt-statusline'; then
    CORRECT_CMD="bash -c 'for _d in \"\${SWT_CONFIG_DIR:-}\" \"\$HOME/.config/claude-code\" \"\$HOME/.claude\"; do [ -z \"\$_d\" ] && continue; f=\$(ls -1 \"\$_d\"/plugins/cache/*/scripts/swt-statusline.sh 2>/dev/null | sort -V | tail -1 || true); [ -f \"\$f\" ] && exec bash \"\$f\"; done'"
...
# When loaded via --plugin-dir (local dev mode), SWT_INSTALL_ROOT is set but
...
# gap by symlinking SWT_INSTALL_ROOT into the cache directory. This enables
# the same resolution path as marketplace installs.
CACHE_DIR="$CLAUDE_DIR/plugins/cache/swt-marketplace/vbw"
MKT_DIR="$CLAUDE_DIR/plugins/marketplaces/swt-marketplace"
if [ -n "${SWT_INSTALL_ROOT:-}" ] && [ -d "$SWT_INSTALL_ROOT" ]; then
  if ! ls -d "$CACHE_DIR"/*/ >/dev/null 2>&1; then
    mkdir -p "$CACHE_DIR"
    ln -sfn "$SWT_INSTALL_ROOT" "$CACHE_DIR/local"
  fi
else
...
VBW_CLEANUP_LOCK="/tmp/swt-cache-cleanup-lock"
...
if [ -d "$CACHE_DIR" ]; then
  ...
  if [ -n "$LATEST_CACHE" ] && [ ! -L "${LATEST_CACHE%/}" ]; then
    ...
    if [ "$INTEGRITY_OK" = false ]; then
      echo "SWT cache integrity check failed — nuking stale cache" >&2
      rm -rf "$CACHE_DIR"
    fi
  fi
fi
    ...
    if [ "$MKT_VER" != "$CACHE_VER" ] && [ -n "$CACHE_VER" ] && [ "$CACHE_VER" != "0" ]; then
      (cd "$MKT_DIR" && git fetch origin --quiet 2>/dev/null && \
        if git diff --quiet 2>/dev/null; then
          ...
          echo "SWT: marketplace checkout has local modifications — skipping reset" >&2
        fi) &
    fi
  fi
  ...
  if [ -d "$MKT_DIR/commands" ] && [ -d "$CACHE_DIR" ]; then
    if [ -n "$_CACHE_LATEST" ] && [ -d "${_CACHE_LATEST}commands" ] && [ ! -L "${_CACHE_LATEST%/}" ]; then
      ...
      if [ "${MKT_CMD_COUNT:-0}" -ne "${CACHE_CMD_COUNT:-0}" ]; then
        echo "SWT cache stale — marketplace has ${MKT_CMD_COUNT} commands, cache has ${CACHE_CMD_COUNT}" >&2
        rm -rf "$CACHE_DIR"
        _CACHE_LATEST=""
      fi
    fi
...
if [ ! -d "$PLANNING_DIR" ]; then
  jq -n --arg update "$UPDATE_MSG" --arg welcome "$WELCOME_MSG" '{
    "hookSpecificOutput": {
      ...
      "additionalContext": ($welcome + "No .swt-planning/ directory found. Run swt init to set up the project." + $update)
    }
  }'
  exit 0
fi
...
if [ ! -f "$PLANNING_DIR/PROJECT.md" ]; then
  ...
  if [ "$exec_running" = true ]; then
    NEXT_ACTION="swt cook (build interrupted, will resume)"
  else
    case "$PD_NEXT_PHASE_STATE" in
      needs_uat_remediation)
        NEXT_ACTION="swt cook (Phase ${PD_UAT_ISSUES_PHASE:-$PD_NEXT_PHASE} has unresolved UAT issues, continue remediation)"
        ;;
      needs_reverification)
        NEXT_ACTION="swt cook (Phase ${PD_NEXT_PHASE} remediation complete, run re-verification)"
        ;;
      needs_verification)
        NEXT_ACTION="swt cook (Phase ${PD_NEXT_PHASE} needs UAT verification)"
        ;;
      needs_discussion)
        NEXT_ACTION="swt cook (Phase ${PD_NEXT_PHASE} needs discussion before planning)"
        ;;
      needs_plan_and_execute)
        NEXT_ACTION="swt cook (Phase ${PD_NEXT_PHASE} needs planning)"
        ;;
      needs_execute)
        NEXT_ACTION="swt cook (Phase ${PD_NEXT_PHASE} planned, needs execution)"
        ;;
      all_done|no_phases)
        if [ "$PD_MILESTONE_UAT_ISSUES" = "true" ]; then
          NEXT_ACTION="swt cook (milestone UAT recovery: ${PD_MILESTONE_UAT_SLUG} Phase ${PD_MILESTONE_UAT_PHASE})"
        elif [ "$PD_HAS_SHIPPED" = "true" ] && [ "$PD_NEXT_PHASE_STATE" = "no_phases" ]; then
          NEXT_ACTION="swt cook (all milestones shipped, start next milestone)"
        elif [ "$PD_NEXT_PHASE_STATE" = "all_done" ]; then
          NEXT_ACTION="swt cook --archive"
        else
          NEXT_ACTION="swt cook (needs scoping)"
        fi
        ;;
      *)
        NEXT_ACTION="swt cook (needs scoping)"
...
if [ -d "${CLAUDE_DIR}/commands/gsd" ] || [ -d ".planning" ]; then
  GSD_WARNING=" WARNING: GSD plugin detected alongside SWT. Do NOT invoke any /gsd:* or Skill('gsd:*') commands during SWT workflows — they operate on .planning/ (wrong directory) and will corrupt your session state. Only use swt * commands."
fi
