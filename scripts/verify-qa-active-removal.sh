...
if grep -q "Next: swt cook (needs scoping)." out1.txt; then
  pass "session-start no-phases next action is needs scoping"
else
  fail "session-start no-phases next action is needs scoping"
fi

mkdir -p .swt-planning/milestones/01-foundation
printf '# SHIPPED\n' > .swt-planning/milestones/01-foundation/SHIPPED.md
...
if grep -q "Next: swt cook (all milestones shipped, start next milestone)." out2.txt; then
