...
has_vbw_context() {
  [ -f "$VBW_PLANNING_DIR/.swt-session" ] \
...
resolve_worktree_map_file() {
  ...
  case "$ROLE" in
    swt-dev|swt-debugger)
      role_matches=$(find "$storage_dir" -maxdepth 1 -type f \( -name "${ROLE#swt-}.json" -o -name "${ROLE#swt-}-*.json" \) 2>/dev/null | sort)
...
rm -f "$VBW_PLANNING_DIR/.cost-ledger.json" "$VBW_PLANNING_DIR/.compaction-marker" "$VBW_PLANNING_DIR/.swt-context" "$VBW_PLANNING_DIR/.context-usage" 2>/dev/null
...
WORKTREE_CONTEXT=""
if echo "$ROLE" | grep -q "swt-dev\|swt-debugger"; then
