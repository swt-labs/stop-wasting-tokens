...
_pd_output_file=
exec 3>&1
if _pd_output_file=$(mktemp "${TMPDIR:-/tmp}/swt-phase-detect.XXXXXX" 2>/dev/null) &&
...
if [ -d "$PHASES_DIR" ]; then
  ...
  if [ "$PHASE_COUNT" -eq 0 ]; then
    ...
    # Priority override: unresolved UAT issues should route first for no-arg swt cook.
...
# QA lifecycle, plain `swt cook` must resume that backlog before drifting into
...
# preserves a deterministic plain `swt cook` resume path for the post-initial-QA
