#!/bin/bash
set -u
# PreCompact hook: Inject SWT-specific compaction priorities
#
...
# SWT-specific compaction priorities per agent role
case "$AGENT_ROLE" in
  scout)
...
# NOTE: Keep this reminder in sync with the Shutdown Handling section in agents/swt-*.md.
case "$AGENT_ROLE" in
