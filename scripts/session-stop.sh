#!/bin/bash
set -u
# Stop hook: Log session metrics to .swt-planning/.session-log.jsonl
...
# Keep .swt-session so plain-text follow-ups in an active SWT flow remain
# unblocked across assistant turns. .swt-session is cleared by explicit
# non-SWT slash commands in prompt-preflight.sh (and stale markers are ignored
...
WORKTREES_DIR="$VBW_CONFIG_ROOT/.swt-worktrees"
