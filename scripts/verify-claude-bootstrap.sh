...
check_absent "greenfield omits Key Decisions (tracked in .swt-planning/)" grep -q '^## Key Decisions$' "$OUT"

# 2) Brownfield preservation + managed section replacement
mkdir -p "$TMP_DIR/.swt-planning"
cat > "$TMP_DIR/.swt-planning/STATE.md" <<'EOF'
# State
...
check_absent "brownfield strips old managed SWT content" grep -q 'OLD MANAGED CONTENT SHOULD BE REPLACED' "$OUT"
...
check "brownfield migrates Key Decisions data to STATE.md" grep -q 'Use widgets' "$TMP_DIR/.swt-planning/STATE.md"
...
VBW_RULES_COUNT="$(grep -c '^## SWT Rules$' "$OUT")"
if [ "$VBW_RULES_COUNT" -eq 1 ]; then
  echo "  PASS  brownfield has one SWT Rules section"
  PASS=$((PASS + 1))
else
  echo "  FAIL  brownfield has one SWT Rules section (found $VBW_RULES_COUNT)"
  FAIL=$((FAIL + 1))
fi
...
check "migrated data row appears in STATE.md" grep -q 'Use widgets' "$TMP_DIR/.swt-planning/STATE.md"
check_absent "placeholder row removed from STATE.md" grep -q 'No decisions yet' "$TMP_DIR/.swt-planning/STATE.md"

# F1/F5: Validate migrated table is contiguous (no blank line between separator and data)
SEPARATOR_LINE=$(grep -n '^|[-|[:space:]]*|$' "$TMP_DIR/.swt-planning/STATE.md" | head -1 | cut -d: -f1)
DATA_LINE=$(grep -n 'Use widgets' "$TMP_DIR/.swt-planning/STATE.md" | head -1 | cut -d: -f1)
...
TODOS_LINE=$(grep -n '^## Todos$' "$TMP_DIR/.swt-planning/STATE.md" | head -1 | cut -d: -f1)
BEFORE_TODOS_LINE=$((TODOS_LINE - 1))
BEFORE_TODOS_CONTENT=$(sed -n "${BEFORE_TODOS_LINE}p" "$TMP_DIR/.swt-planning/STATE.md")
...
check "placeholder row preserved in STATE.md for empty table" grep -q 'No decisions yet' "$TMP_DIR/.swt-planning/STATE.md"
...
WIDGET_COUNT=$(grep -c 'Use widgets' "$TMP_DIR/.swt-planning/STATE.md")
...
check "deduplication: new row added to STATE.md" grep -q 'New decision' "$TMP_DIR/.swt-planning/STATE.md"
...
check "mixed: table data migrated to STATE.md" grep -q 'Use widgets' "$TMP_DIR/.swt-planning/STATE.md"
...
check "trailing whitespace: data migrated to STATE.md" grep -q 'Use widgets' "$TMP_DIR/.swt-planning/STATE.md"
