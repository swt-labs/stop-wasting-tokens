...
# refresh-claude-md-swt-sections.sh — Refresh SWT-owned CLAUDE.md sections
# without rebuilding arbitrary user-authored content.
#
# Usage:
#   refresh-claude-md-swt-sections.sh CLAUDE.md
...
if [ "$#" -ne 1 ]; then
  echo "Usage: refresh-claude-md-swt-sections.sh CLAUDE.md" >&2
  exit 1
fi
...
if vbw_should_emit_managed_section "$CLAUDE_MD" "SWT Rules" "## SWT Rules"; then
