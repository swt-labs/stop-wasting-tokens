...
#   command: the SWT command that just ran (implement, qa, plan, execute, fix, etc.)
...
suggest_milestone_recovery() {
  if [ "$milestone_uat_issues" = true ]; then
    if [ "$milestone_uat_slug" != "none" ] && [ "${milestone_uat_count:-0}" -gt 1 ] 2>/dev/null; then
      suggest "swt cook -- Milestone UAT recovery pending (${milestone_uat_slug}, ${milestone_uat_count} phase(s))"
    elif [ "$milestone_uat_slug" != "none" ] && [ "$milestone_uat_phase" != "none" ]; then
      suggest "swt cook -- Milestone UAT recovery pending (${milestone_uat_slug}, Phase ${milestone_uat_phase})"
    else
      suggest "swt cook -- Milestone UAT recovery pending"
    fi
    return 0
  fi
  return 1
...
case "$CMD" in
  init)
    suggest "swt cook -- Define your project and start building"
    ;;
    ...
    case "$effective_result" in
      fail)
        if [ -n "$failing_plan_ids" ]; then
          ...
          suggest "swt fix -- Fix plan $first_fail (failed verification)"
        else
          suggest "swt fix -- Fix the failing checks"
        fi
        suggest "swt cook -- Re-run verification after fixing"
        ;;
      partial)
        if [ -n "$failing_plan_ids" ]; then
          first_fail=$(echo "$failing_plan_ids" | awk '{print $1}')
          suggest "swt fix -- Fix plan $first_fail (partial failure)"
        else
          suggest "swt fix -- Address partial failures"
        fi
        if [ "$all_done" != true ]; then
          suggest "swt cook -- Continue to next phase"
        fi
        ;;
      *)
        ...
        if [ -z "$current_uat_issues_phase" ] && \
          ...
          if [ -n "$first_unverified_phase" ]; then
            ...
            suggest "swt cook -- Verify $_uv_label before continuing"
          else
            suggest "swt cook -- Verify completed work before continuing"
          fi
        fi
        if [ "$all_done" = true ] || [ "$next_phase_state" = "needs_reverification" ] || { [ "$next_phase_state" = "needs_verification" ] && [ -n "$current_uat_blocking_phase" ]; }; then
          if [ "$next_phase_state" = "needs_reverification" ]; then
            suggest "swt cook -- Re-verify Phase ${pd_next_phase:-} after remediation"
          elif [ "$next_phase_state" = "needs_verification" ] && [ -n "$current_uat_blocking_phase" ]; then
            suggest "swt cook -- Resume Verify for Phase ${pd_next_phase:-}"
          elif ! suggest_milestone_recovery; then
            if [ "$deviation_count" -eq 0 ]; then
              suggest "swt cook --archive -- All phases complete, zero deviations"
            else
              suggest "swt cook --archive -- Archive completed work ($deviation_count deviation(s) logged)"
              suggest "swt cook -- Review before archiving"
            fi
          elif [ "$deviation_count" -gt 0 ]; then
            suggest "swt cook -- Review remediation scope before resuming new work"
          else
            :
          fi
        elif [ -n "$current_uat_issues_phase" ]; then
          if [ "$current_uat_major_or_higher" = true ]; then
            suggest "swt cook -- Remediate UAT issues for $current_uat_issues_label"
          else
            suggest "swt fix -- Fix minor UAT issues in $current_uat_issues_label"
          fi
          ...
          if [ -n "$next_undiscussed" ] && [ "$next_undiscussed" = "$target" ]; then
            suggest "swt discuss $target -- Discuss phase before planning"
            ...
            for dir in "$PHASES_DIR"/*/; do
              ...
              if [ "$pn" = "$target" ]; then
                ...
                suggest "swt cook -- Plan Phase $target: $(fmt_phase_name "$tname") (discussion pre-seeded from UAT)"
                break
              fi
            done
            ...
            for dir in "$PHASES_DIR"/*/; do
              ...
              if [ "$pn" = "$target" ]; then
                tname=$(basename "$dir" | sed 's/^[0-9]*-//')
                suggest "swt cook -- Continue to Phase $target: $(fmt_phase_name "$tname")"
                break
              fi
            done
          else
        ...
        if [ "$RESULT" = "skipped" ]; then
          suggest "swt cook -- Verify completed work"
        fi
        ;;
    esac
    ;;
  ...
  plan)
    if [ "$active_phase_plans" -gt 0 ]; then
      suggest "swt cook -- Execute $active_phase_plans plans ($effort effort)"
    else
      suggest "swt cook -- Execute the planned phase"
    fi
    ;;
  ...
  qa)
                ...
                pass)
                  case "$_qa_ds_status" in
                    qa_pending|fix_applied)
                      suggest "swt debug --resume -- Continue to QA verification"
                      _qa_debug_handled=true
                      ;;
                    uat_pending)
                      suggest "swt debug --resume -- Continue to UAT verification"
                      _qa_debug_handled=true
                      ;;
                  esac
                  ;;
                fail|partial)
                  suggest "swt debug --resume -- Address QA failures"
                  _qa_debug_handled=true
                  ;;
                *)
    ...
    case "$effective_result" in
      ...
      fail)
        if [ -n "$failing_plan_ids" ]; then
          first_fail=$(echo "$failing_plan_ids" | awk '{print $1}')
          suggest "swt fix -- Fix plan $first_fail (failed QA)"
        else
          suggest "swt fix -- Fix the failing checks"
        fi
        ;;
      partial)
        ...
        suggest "swt cook -- Continue despite warnings"
        ;;
      *)
        suggest "swt cook -- Continue building"
        ;;
    esac
    fi
    ;;
  ...
  fix)
      ...
      if [ -n "$_fix_ds_output" ]; then
        ...
        if [ "${active_session:-none}" != "none" ] && [ -n "${session_file:-}" ] && [ -f "$session_file" ]; then
          ...
          case "$_fix_ds_status" in
            qa_pending|fix_applied)
              suggest "swt debug --resume -- Address remaining issues"
            ...
            qa_failed)
              ...
              ;;
            uat_pending)
              suggest "swt debug --resume -- Run UAT on the debug fix"
              _fix_debug_handled=true
              ;;
            uat_failed)
              suggest "swt debug --resume -- Address UAT issues"
              _fix_debug_handled=true
              ;;
            investigating)
              suggest "swt debug --resume -- Continue investigation"
              _fix_debug_handled=true
              ;;
          esac
        fi
    ...
    if [ "$_fix_debug_handled" = false ]; then
      suggest "swt cook -- Verify the fix"
      if [ "$cfg_auto_uat" = true ] && [ -f "$PLANNING_DIR/.last-fix-commit" ]; then
        ...
        if [ "$_marker_stale" = false ]; then
          suggest "swt verify -- Run UAT on the fix"
        fi
      fi
      suggest "swt cook -- Continue building"
    fi
    ...
    case "$effective_result" in
      pass)
        if [ "$next_phase_state" = "needs_reverification" ]; then
          ...
          if ! suggest_milestone_recovery; then
            suggest "swt cook --archive -- All verified, ready to ship"
          fi
        else
          suggest "swt cook -- Continue to next phase"
        fi
      ...
      issues_found)
        if [ "$uat_major_or_higher" = true ]; then
          if [ -n "$verify_target_phase" ]; then
            suggest "swt cook -- Remediate UAT issues for Phase $verify_target_phase"
          else
            suggest "swt cook -- Remediate UAT issues from the latest report"
          fi
        else
          suggest "swt fix -- Fix minor issues found during UAT"
        fi
        ;;
      *)
        suggest "swt cook -- Continue building"
  ...
  debug)
    ...
    case "$_ds_status" in
      ...
      uat_failed)
        ...
        ;;
      complete)
        suggest "swt status -- View project status"
        ;;
      investigating)
        suggest "swt debug --resume -- Continue investigation"
        suggest "swt fix -- Apply the fix"
        ;;
      *)
        suggest "swt fix -- Apply the fix"
        suggest "swt cook -- Continue building"
  ...
  config)
    if [ "$has_project" = true ]; then
      suggest "swt status -- View project state"
    else
      suggest "swt cook -- Define your project and start building"
    fi
    ;;
  ...
  archive)
    suggest "swt cook -- Start new work"
    ;;
  ...
  status)
    if [ "$next_phase_state" = "needs_reverification" ]; then
      ...
      if [ -n "$first_unverified_phase" ]; then
        ...
        suggest "swt cook -- Verify completed phases before continuing"
      fi
    elif [ "$all_done" = true ]; then
      if ! suggest_milestone_recovery; then
        if [ "$deviation_count" -eq 0 ]; then
          ...
          suggest "swt cook --archive -- Archive completed work"
        fi
      else
        :
      fi
      ...
      if [ -n "$next_undiscussed" ] && [ "$next_undiscussed" = "$target" ]; then
        ...
        for dir in "$PHASES_DIR"/*/; do
          ...
          if [ "$pn" = "$target" ]; then
            tname=$(basename "$dir" | sed 's/^[0-9]*-//')
            suggest "swt cook -- Continue Phase $target: $(fmt_phase_name "$tname")"
            break
          fi
        done
      fi
    else
      suggest "swt cook -- Start building"
    fi
    ;;
  ...
  discuss|assumptions)
    suggest "swt cook --plan -- Plan this phase"
    suggest "swt cook -- Plan and execute in one flow"
    ;;
  ...
  resume)
    ...
    suggest "swt status -- View current progress"
    ;;
  ...
  *)
    ...
    if [ "$has_project" = true ]; then
      suggest "swt cook -- Continue building"
      suggest "swt status -- View project progress"
    else
      suggest "swt cook -- Start a new project"
    fi
    ;;
esac
  ...
  *)
    if [ "$has_project" = true ] && [ "$phase_count" -gt 0 ]; then
      if [ "$map_exists" = false ]; then
        suggest "swt map -- Map your codebase for better planning"
      elif [ "$map_staleness" -gt 30 ]; then
        suggest "swt map --incremental -- Codebase map is ${map_staleness}% stale"
      fi
    fi
    ;;
esac
  ...
  *)
    if [ "$has_project" = true ] && [ -d "$PHASES_DIR" ]; then
      ...
      if [ "$_ki_total" -gt 0 ]; then
        echo ""
        echo "Note: $_ki_total known issue(s) deferred across phases — visible in swt list-todos"
      fi
    fi
    ;;
esac
