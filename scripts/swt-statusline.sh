...
_UID=$(id -u)
_OS=$(uname)
# Derive script dir early — used as a preferred anchor for find_swt_root (#266).
# In dev (--plugin-dir), the script lives inside the project repo; anchoring here
# makes .swt-planning/ resolution immune to agents cd-ing around the monorepo.
...
. "$_SL_SCRIPT_DIR/lib/swt-config-root.sh"
# shellcheck source=lib/swt-cache-key.sh
. "$_SL_SCRIPT_DIR/lib/swt-cache-key.sh"
find_swt_root "$_SL_SCRIPT_DIR"
...
if ! [ -f "${_CACHE}-ok" ] || ! [ -O "${_CACHE}-ok" ]; then
  ...
  rm -f /tmp/swt-sl-cache-"${_UID}" /tmp/swt-usage-cache-"${_UID}" /tmp/swt-gh-cache-"${_UID}" /tmp/swt-team-cache-"${_UID}" /tmp/swt-*-"${_UID}" 2>/dev/null
  # Clean old-format caches that lack the repo hash (e.g. swt-1.0.0-501-fast)
  for f in /tmp/swt-*-"${_UID}"-fast /tmp/swt-*-"${_UID}"-slow /tmp/swt-*-"${_UID}"-cost /tmp/swt-*-"${_UID}"-ok; do
    # Old format: swt-{ver}-{uid}-{suffix} (3 dashes). New format: swt-{ver}-{uid}-{hash}-{suffix} (4 dashes).
    _stale_bn="${f##*/}"
...
_resolve_notraffic() {
  ...
  if [ -z "$_val" ]; then
    local _sdir
    for _sdir in "${SWT_CONFIG_DIR:-}" "$HOME/.config/claude-code" "$HOME/.claude"; do
      [ -z "$_sdir" ] && continue
...
if [ -d "$SWT_PLANNING_DIR" ]; then
  printf '%s\n' "${SWT_SESSION_ID:-unknown}|${PCT}|${CTX_SIZE}" > "$SWT_PLANNING_DIR/.context-usage" 2>/dev/null || true
fi
...
# --- Fast cache (5s TTL): SWT state + execution + agents ---
FAST_CF="${_CACHE}-fast"

if ! cache_fresh "$FAST_CF" 5 || lifecycle_artifacts_newer_than_cache "$FAST_CF" "$SWT_PLANNING_DIR"; then
  ...
  if [ -f "$SWT_PLANNING_DIR/STATE.md" ]; then
    ...
    _phase_line=$(grep -m1 "^Phase:" "$SWT_PLANNING_DIR/STATE.md" 2>/dev/null)
    PH=$(echo "$_phase_line" | sed -n 's/^Phase:[[:space:]]*\([0-9][0-9]*\).*/\1/p')
    TT=$(echo "$_phase_line" | sed -n 's/.*[[:space:]]of[[:space:]]*\([0-9][0-9]*\).*/\1/p')
  fi
  if [ -f "$SWT_PLANNING_DIR/config.json" ]; then
    # Auto-migrate: add model_profile if missing
    if ! jq -e '.model_profile' "$SWT_PLANNING_DIR/config.json" >/dev/null 2>&1; then
      TMP=$(mktemp)
      jq '. + {model_profile: "quality", model_overrides: {}}' "$SWT_PLANNING_DIR/config.json" > "$TMP" && mv "$TMP" "$SWT_PLANNING_DIR/config.json"
    fi
    EF=$(jq -r '.effort // "balanced"' "$SWT_PLANNING_DIR/config.json" 2>/dev/null)
    MP=$(jq -r '.model_profile // "quality"' "$SWT_PLANNING_DIR/config.json" 2>/dev/null)
    HIDE_AGENT_TMUX=$(jq -r '.statusline_hide_agent_in_tmux // false' "$SWT_PLANNING_DIR/config.json" 2>/dev/null)
    COLLAPSE_AGENT_TMUX=$(jq -r '.statusline_collapse_agent_in_tmux // false' "$SWT_PLANNING_DIR/config.json" 2>/dev/null)
  fi
  ...
  if [ -d "$SWT_PLANNING_DIR/phases" ]; then
    PT=0
    PD=0
    ...
    done < <(list_canonical_phase_dirs "$SWT_PLANNING_DIR/phases")
    if [ -n "$PH" ] && [ "$PH" != "0" ]; then
      PDIR=$(find_phase_dir_by_ref "$SWT_PLANNING_DIR" "$PH")
  ...
  if [ -f "$SWT_PLANNING_DIR/.execution-state.json" ]; then
    IFS='|' read -r EXEC_STATUS EXEC_WAVE EXEC_TWAVES EXEC_DONE EXEC_TOTAL EXEC_CURRENT <<< \
      ...
      ] | join("|")' "$SWT_PLANNING_DIR/.execution-state.json" 2>/dev/null)"
    ...
    if [ "$EXEC_STATUS" = "running" ] && [ "${EXEC_DONE:-0}" -gt 0 ] 2>/dev/null; then
      _exec_phase=$(jq -r '.phase // ""' "$SWT_PLANNING_DIR/.execution-state.json" 2>/dev/null)
      if [ -n "$_exec_phase" ]; then
        _exec_pdir=$(find_phase_dir_by_ref "$SWT_PLANNING_DIR" "$_exec_phase")
...
SWT_CTX=0; [ -f "$SWT_PLANNING_DIR/.swt-context" ] && SWT_CTX=1
if [ "$SWT_CTX" = "1" ]; then
  VC="${C}${B}"
else
  VC="${D}"
...
if [ -n "${TMUX:-}" ]; then
  ...
  if [ -n "$_GIT_DIR" ] && [ -n "$_GIT_COMMON" ] && [ "$_GIT_DIR" != "$_GIT_COMMON" ]; then
    ...
    _COLLAPSE_WT=$(jq -r '.statusline_collapse_agent_in_tmux // false' \
      "$_MAIN_ROOT/.swt-planning/config.json" 2>/dev/null)
...
if ! cache_fresh "$SLOW_CF" "$_SLOW_TTL"; then
  ...
  AUTH_METHOD=""
  AUTH_CLASS="api_key"
  HIDE_LIMITS=$(jq -r '.statusline_hide_limits // false' "$SWT_PLANNING_DIR/config.json" 2>/dev/null)
  HIDE_LIMITS_API=$(jq -r '.statusline_hide_limits_for_api_key // false' "$SWT_PLANNING_DIR/config.json" 2>/dev/null)

  # Priority 1: env var override (escape hatch for keychain issues)
  if [ -n "${SWT_OAUTH_TOKEN:-}" ]; then
    OAUTH_TOKEN="$SWT_OAUTH_TOKEN"
    AUTH_CLASS="oauth"
  fi

  # Priority 2: system credential store (skip if SWT_SKIP_KEYCHAIN=1, e.g. in tests)
  if [ -z "$OAUTH_TOKEN" ] && [ "${SWT_SKIP_KEYCHAIN:-0}" != "1" ]; then
    if [ "$_OS" = "Darwin" ]; then
  ...
  # When SWT_SKIP_KEYCHAIN=1 (e.g. in tests), only check the explicitly-set
  # SWT_CONFIG_DIR — skip hardcoded fallback paths that may hold real credentials.
  if [ -z "$OAUTH_TOKEN" ]; then
    if [ "${SWT_SKIP_KEYCHAIN:-0}" = "1" ]; then
      _p3_dirs=("${SWT_CONFIG_DIR:-}")
    else
      _p3_dirs=("${SWT_CONFIG_DIR:-}" "$HOME/.config/claude-code" "$HOME/.claude")
    fi
  ...
  # Skip if SWT_SKIP_AUTH_CLI=1 (e.g. in tests on dev machines with real auth)
  if [ -z "$OAUTH_TOKEN" ] && [ "${SWT_SKIP_AUTH_CLI:-0}" != "1" ]; then
  ...
  case "${SWT_SKIP_UPDATE_CHECK:-0}" in
...
COST_CF="${_CACHE}-cost"
LEDGER_FILE="$SWT_PLANNING_DIR/.cost-ledger.json"
PREV_COST=""
...
if acquire_lock_dir "$_COST_LOCK_DIR"; then
  ...
  if [ -n "$PREV_COST" ] && [ -d "$SWT_PLANNING_DIR" ]; then
  _to_cents() {
    local val="$1" w f
    w="${val%%.*}"
  ...
  if [ "$DELTA_CENTS" -gt 0 ]; then
    ACTIVE_AGENT="other"
    [ -f "$SWT_PLANNING_DIR/.active-agent" ] && ACTIVE_AGENT=$(cat "$SWT_PLANNING_DIR/.active-agent" 2>/dev/null)
...
if [ "$FETCH_OK" = "ok" ]; then
  ...
  USAGE_LINE="${D}Limits: keychain access denied (allow Terminal in Keychain Access.app or set SWT_OAUTH_TOKEN)${X}"
