...
PLANNING_DIR="${VBW_PLANNING_DIR:-$(pwd)/.swt-planning}"
...
scan_stale_teams() {
  ...
  for team_dir in "$TEAMS_DIR"/*; do
    ...
    # Only report SWT-owned teams — non-SWT configless dirs may belong to other plugins
    if [ ! -f "$team_dir/config.json" ]; then
      case "$team_name" in swt-*)
    ...
    # Only scan SWT-owned teams for staleness — consistent with cleanup scope
    case "$team_name" in swt-*) ;; *) continue ;; esac
...
scan_stale_worktrees() {
  local worktrees_dir
  worktrees_dir="$(pwd)/.swt-worktrees"
