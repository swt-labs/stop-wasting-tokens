#!/usr/bin/env bash
# resolve-agent-model.sh - Model resolution for SWT agents
#
...
#   config-path: path to .swt-planning/config.json
...
#   MODEL=$(bash ${SWT_INSTALL_ROOT}/scripts/resolve-agent-model.sh lead .swt-planning/config.json ${SWT_INSTALL_ROOT}/config/model-profiles.json)
...
if [ ! -f "$CONFIG_PATH" ]; then
  echo "Config not found at $CONFIG_PATH. Run swt init first." >&2
  exit 1
fi
...
CACHE_FILE="/tmp/swt-model-${AGENT}-${PATH_HASH}-${CONFIG_HASH}-${PROFILES_HASH}"
