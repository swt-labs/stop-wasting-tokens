...
# shellcheck source=scripts/lib/swt-target-root.sh
. "${SCRIPT_DIR}/lib/swt-target-root.sh"
...
if [ -n "$TARGET_ROOT" ] && [ -d "$TARGET_ROOT/.swt-planning" ] && [ ! -d "$PLANNING_DIR" ]; then
  PLANNING_DIR="$TARGET_ROOT/.swt-planning"
fi
...
# Execution must happen in active phases (.swt-planning/phases/), not archived milestones.
case "$PHASES_DIR" in
  */.swt-planning/milestones/*|.swt-planning/milestones/*)
    ...
    echo "Execution must target active phases in .swt-planning/phases/" >&2
    exit 1
    ;;
esac

# --- Update context index entry (REQ-04) ---
# Writes/updates an entry in .swt-planning/.cache/context-index.json
...
emit_codebase_mapping_hint() {
  ...
  echo "Codebase mapping exists in \`.swt-planning/codebase/\`. Key files:"
