...
path_is_code_fix_support_artifact() {
  local path="${1:-}"
  case "$path" in
    .swt-planning/*|.claude/*|.claude-plugin/*)
      return 0
      ;;
  esac
  if path_is_recorded_non_code_artifact "$path"; then
...
resolve_corroborated_recorded_paths() {
  ...
  while IFS= read -r path; do
    ...
    # .swt-planning/ paths when planning_tracking=ignore).
...
# .swt-planning/ in .gitignore — the normal diff/worktree sets exclude
...
git_ignored_metadata_worktree_paths() {
  ...
  while IFS= read -r path; do
    ...
    -- .swt-planning .claude 2>/dev/null || true)
}
...
# .swt-planning/ files (no production code), phase-level deviations
