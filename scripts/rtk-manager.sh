...
# rtk-manager.sh — Explicit, opt-in RTK management for SWT.
#
...
# require --yes or --dry-run so SWT never changes global RTK/Claude Code state
...
Smoke helpers are explicit runtime verification internals used by swt rtk verify; they write only local SWT RTK pending/proof/failure JSON.
EOF
}
...
fetch_latest_release() {
  ...
  [ -n "$target" ] || die 1 "unsupported platform for SWT-managed RTK release install"
...
smoke_status_preconditions() {
  ...
  printf '%s' "$payload" | jq -e '.config_state == "present"' >/dev/null 2>&1 || smoke_fail 1 "RTK config is not present" "run swt rtk install or swt rtk init first"
...
smoke_start() {
  ...
  claude_session_id="${SWT_SESSION_ID:-}"
...
smoke_finish() {
  ...
  if [ "$history_validation_failed" = "false" ] && [ -n "$history_before_tail" ] && [ -z "$history_evidence" ]; then
    if [ -z "$before_ls" ] || [ -z "$before_status" ] || [ -z "$before_log" ]; then
      smoke_fail 1 "fresh smoke evidence requires a new swt rtk verify attempt" "pending smoke file lacks history_before_command_counts"
    fi
  ...
  if [ "$history_validation_failed" = "true" ]; then
    ...
    if [ -n "$transcript_evidence_json" ]; then
      verify_bash_guard_smoke || smoke_fail 1 "SWT Bash guard smoke verification failed" "expected scripts/bash-guard.sh to block synthetic destructive command with exit 2"
...
status_json() {
  ...
  if [ "$updated_input_risk" = "true" ]; then
    ...
    if [ "$compatibility" = "verified" ]; then
      diagnostic_caveat="Claude Code issue #15897 remains an upstream diagnostic caveat; local runtime smoke proof verifies this RTK/SWT setup."
    fi
  fi
  ...
  case "$compatibility" in
    absent)
      if [ "$binary_install_state" = "installed_not_on_path" ]; then
        summary="RTK binary installed by SWT but not on PATH; add $(dirname "$receipt_binary") to PATH before hook activation"
        next_action="path"
      elif [ "$binary_install_state" = "receipt_missing_binary" ]; then
        summary="SWT RTK receipt exists but binary is missing; run swt rtk install or uninstall/manage"
        next_action="repair"
      else
        summary="RTK not installed; optional: swt rtk install"
        next_action="install"
      fi
      ;;
    settings_unreadable)
    ...
    risk)
      ...
      ;;
    verified)
      summary="RTK/SWT coexistence verified by runtime smoke proof"
      next_action="status"
      ;;
    *)
...
doctor_json() {
  status_json false false | jq '
    ...
    | ([
        ...
        (if (($s.install_receipt // "") != "") then "SWT install receipt" else empty end),
    ...
    | .doctor_detail = (
        ...
        elif (.config_state == "missing" and ((.rtk_present == true) or (.global_hook_present == true))) then "RTK config missing at " + ($s.config_path // "unknown") + "; run swt rtk install or swt rtk init to repair setup"
        elif .update_available == true then "outdated from cached RTK release check; run swt rtk update"
        elif .doctor_status == "PASS" then "verified by runtime smoke proof: " + ($s.proof_source // "smoke proof")
        elif ($rtk_artifacts | length) > 0 then "RTK artifacts present with no active settings hook: " + ($rtk_artifacts | join(", ")) + "; run swt rtk status or swt rtk uninstall/manage"
        elif .compatibility == "absent" then "not installed; optional: swt rtk install"
...
print_install_preflight() {
  ...
  if [ "$method" = "homebrew" ]; then
    ...
    if [ "$operation" = "update" ]; then
      ...
      next_step="Run swt rtk verify if hook compatibility needs re-checking."
    else
      will_run="brew install rtk; rtk --version; rtk gain; rtk config --create when config is missing; rtk init -g --auto-patch"
      writes="Homebrew-managed RTK files; receipt ${RTK_RECEIPT_FILE}; config $(rtk_config_path); ${RTK_SETTINGS_JSON}, ${RTK_GLOBAL_MD}, ${RTK_CLAUDE_MD} when RTK/SWT patch hooks"
      ...
      next_step="Run swt rtk verify after restart."
    fi
  else
    ...
    if [ "$operation" = "update" ]; then
      ...
      next_step="Run swt rtk verify after restart; add ${install_dir} to PATH only for future manual rtk shell usage."
    fi
  fi
...
activate_rtk_hook() {
  ...
  if [ "$recoverable" = "true" ] && settings_valid; then
    ...
    if [ "$hook_present" = "true" ] && status_hook_matches_rtk_path "$status_after" "$rtk_path"; then
      rm -f "$stdout_file" "$stderr_file"
      echo "Hook: active via SWT settings fallback patch"
      return 0
    fi
  fi
  ...
  [ "$status" -eq 0 ] || die "$status" "rtk init -g --auto-patch failed before SWT could verify or patch the settings hook"
  ...
  if [ "$operation" = "update" ]; then
    ...
    if receipt_managed; then
      ...
      if [ -z "$receipt_binary" ] || [ ! -x "$receipt_binary" ]; then
        ...
        echo "Method: SWT receipt exists but its binary is missing"
        echo "Will run: no update"
        echo "Next step: run swt rtk install or repair the receipt-owned binary path."
        exit 1
      fi
      ...
      echo "Will run: no SWT-managed overwrite and no ownership takeover"
      ...
      echo "Will run: no update"
      echo "Next step: run swt rtk install."
      exit 0
    fi
  fi
...
init_hook() {
  ...
  if [ -z "$rtk_path" ]; then
    ...
    die 1 "RTK binary not found on PATH; run swt rtk install first or install RTK manually"
  fi
  ...
  echo "Writes: config $(rtk_config_path) when missing; ${RTK_SETTINGS_JSON}, ${RTK_GLOBAL_MD}, and @RTK.md references inside ${RTK_CLAUDE_MD} when RTK/SWT patches Claude Code"
  ...
  echo "Next step: restart Claude Code after hook activation, then run swt rtk verify"
  echo "Risk: Claude Code issue #15897 reports updatedInput can fail when multiple PreToolUse hooks match; compatibility remains unverified until a runtime smoke proof exists. SWT bash-guard.sh is not disabled, reordered, or weakened."
...
verify_rtk() {
  ...
  status_json false false | jq -r '
    ...
    (if .compatibility == "verified" then "" else "\nManual smoke: run the scoped swt rtk verify Bash-tool sequence; if it fails, inspect the RTK smoke failure diagnostic and rerun swt rtk verify after addressing the local evidence gap." end)
  '
}
...
uninstall_rtk() {
  ...
  if [ "$managed" = "true" ] && [ "$receipt_method" = "homebrew" ]; then
    ...
    echo "Will remove: $binary_path (SWT receipt-owned)"
  else
    echo "Will remove: no binary; external installs are not deleted by SWT"
  fi
  if [ "$managed" = "true" ] && { [ -n "$active_hook" ] || [ "$settings_unknown" = "true" ]; } && [ "$deactivate_hook" != "true" ]; then
    ...
    will_run="remove SWT receipt-owned binary only; no hook deactivation requested"
  else
  ...
  [ "$dry_run" = "true" ] && exit 0
  if [ "$managed" = "true" ] && { [ -n "$active_hook" ] || [ "$settings_unknown" = "true" ]; } && [ "$deactivate_hook" != "true" ]; then
    echo "RTK: active or unreadable RTK settings hook state detected; pass --deactivate-hook before removing the SWT-managed binary" >&2
    exit 1
  fi
