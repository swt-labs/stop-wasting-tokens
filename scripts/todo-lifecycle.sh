...
RAW_SESSION_KEY="${SWT_SESSION_ID:-default}"
SESSION_KEY="$(printf '%s' "$RAW_SESSION_KEY" | tr -c 'A-Za-z0-9_.-' '_')"
SESSION_KEY="${SESSION_KEY:-default}"
SNAPSHOT_PATH="/tmp/.swt-last-list-view-${SESSION_KEY}.json"
...
snapshot_save() {
  ...
  if ! printf '%s' "$json_input" | jq empty >/dev/null 2>&1; then
    error_json "snapshot_invalid" "Todo snapshot JSON is invalid. Rerun swt list-todos."
    return 0
  fi

  if ! snapshot_validate_schema "$json_input"; then
    error_json "snapshot_invalid" "Todo snapshot JSON is malformed. Rerun swt list-todos."
    return 0
  fi

  tmp_file=$(mktemp "${SNAPSHOT_PATH}.tmp.XXXXXX" 2>/dev/null) || {
    error_json "snapshot_write_failed" "Todo snapshot could not be created. Rerun swt list-todos."
    return 0
  }

  if ! printf '%s' "$json_input" | jq -cS '.' > "$tmp_file"; then
    rm -f "$tmp_file"
    error_json "snapshot_write_failed" "Todo snapshot could not be written. Rerun swt list-todos."
    return 0
  fi
  ...
  if ! mv "$tmp_file" "$SNAPSHOT_PATH"; then
    rm -f "$tmp_file"
    error_json "snapshot_write_failed" "Todo snapshot could not be finalized. Rerun swt list-todos."
    return 0
  fi
...
snapshot_show() {
  if [ ! -f "$SNAPSHOT_PATH" ]; then
    error_json "snapshot_missing" "Todo snapshot missing — rerun swt list-todos first."
    return 0
  fi
  ...
  if ! printf '%s' "$json_input" | jq empty >/dev/null 2>&1; then
    error_json "snapshot_invalid" "Todo snapshot is malformed — rerun swt list-todos first."
    return 0
  fi
...
archived_state_message() {
  echo "This todo came from archived milestone state. Restore the writable root STATE.md first by restarting so session-start.sh can run migration, or run 'bash scripts/migrate-orphaned-state.sh .swt-planning'."
}

state_path_is_archived() {
  local state_path="$1"
  case "$state_path" in
    */.swt-planning/milestones/*|.swt-planning/milestones/*) return 0 ;;
    *) return 1 ;;
  esac
}
...
snapshot_select() {
  ...
  if [ -z "$selection" ] || ! [[ "$selection" =~ ^[0-9]+$ ]]; then
    error_json "invalid_selection" "Invalid selection — choose a numbered todo from swt list-todos."
    return 0
  fi
  ...
  if [ "$require_unfiltered" = "true" ] && [ -n "$filter" ]; then
    error_json "snapshot_filtered" "Current list view is filtered — rerun unfiltered swt list-todos before using this numbered todo command."
    return 0
  fi
...
validate_item_against_live() {
  ...
  if [ -z "$state_path" ] || [ -z "$section_name" ] || [ -z "$expected_occurrence" ] || [ -z "$expected_total" ]; then
    error_json "invalid_item" "Todo selection payload is missing required metadata. Rerun swt list-todos."
    return 0
  fi

  if [ ! -f "$state_path" ]; then
    error_json "state_missing" "Todo selection no longer matches live backlog. Rerun swt list-todos."
    return 0
  fi
  ...
  if [ "$candidate_count" -ne 1 ] 2>/dev/null; then
    error_json "selection_stale" "Todo selection no longer matches live backlog. Rerun swt list-todos."
    return 0
  fi
...
validate_item_cmd() {
  ...
  if ! printf '%s' "$item_json" | jq empty >/dev/null 2>&1; then
    error_json "invalid_item" "Todo selection payload is invalid. Rerun swt list-todos."
    return 0
  fi
...
detail_warning() {
  ...
  if ! append_activity_entry "$state_path" "Detail for ref ${ref_hash} could not be loaded"; then
    error_json "activity_write_failed" "Could not record the detail warning in STATE.md. Rerun swt list-todos or inspect STATE.md manually."
    return 0
  fi
...
rewrite_state_for_item() {
  ...
  if ! direct_count=$(extract_section_lines "$state_path" "$section_name" 2>/dev/null | sed '/^$/d' | wc -l | tr -d ' '); then
    error_json "rewrite_prepare_failed" "Could not inspect the current todo section in STATE.md. Rerun swt list-todos."
    return 0
  fi
  tmp_file=$(make_temp_with_suffix "$state_path" rewrite) || {
    error_json "rewrite_prepare_failed" "Could not prepare a temporary STATE.md rewrite. Rerun swt list-todos."
    return 0
  }
  ...
  if [ "$direct_count" -le 1 ] 2>/dev/null; then
    ...
    ' "$state_path" > "$tmp_file" || {
      rm -f "$tmp_file"
      error_json "rewrite_prepare_failed" "Could not stage the updated todo section. Rerun swt list-todos."
      return 0
    }
  else
    awk -v target="$line_no" '
  ...
  if ! append_activity_entry "$tmp_file" "$activity_message"; then
    rm -f "$tmp_file"
    error_json "activity_write_failed" "Could not append the activity breadcrumb to STATE.md. Rerun swt list-todos."
    return 0
  fi
  if ! mv "$tmp_file" "$state_path"; then
    rm -f "$tmp_file"
    error_json "rewrite_finalize_failed" "Could not finalize the STATE.md rewrite. Rerun swt list-todos."
    return 0
  fi
...
mutate_item() {
  ...
  case "$mode" in
    ...
    remove) activity_text="Removed todo via swt list-todos: ${activity_text}" ;;
...
case "$CMD" in
  ...
  pickup)
    ...
    ;;
  remove)
    mutate_item "remove" "swt list-todos" "${1:-none}" "${2:-keep}"
    ;;
  *)
    usage
    ;;
