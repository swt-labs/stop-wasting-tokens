...
#     -> validates explicit swt config input. Emits canonical JSON literal
...
resolve_from_config() {
  local config_path="${1:-.swt-planning/config.json}"
  local raw canonical
...
case "${1:-}" in
  ...
  --help|-h)
    ...
    ;;
  *)
    resolve_from_config "${1:-.swt-planning/config.json}"
    ;;
esac