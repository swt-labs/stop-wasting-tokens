...
find_project_root() {
  local dir="$PWD"
  while [ "$dir" != "/" ]; do
    if [ -d "$dir/.swt-planning/phases" ]; then
      printf '%s\n' "$dir"
      return 0
    fi
    dir=$(dirname "$dir")
...
resolve_project_root() {
  ...
  if [ -f "$SCRIPT_DIR/lib/swt-config-root.sh" ]; then
    # shellcheck source=lib/swt-config-root.sh
    if source "$SCRIPT_DIR/lib/swt-config-root.sh" 2>/dev/null; then
...
MARKER_STATUS=$(VBW_PLANNING_DIR="$PROJECT_ROOT/.swt-planning" bash "$SCRIPT_DIR/delegated-workflow.sh" status-json 2>/dev/null) || exit 0
...
requested_vbw_worktree_cwd() {
  requested_cwd_values | grep -Eq '(^|/)\.swt-worktrees(/|$)'
}

EXEC_STATE_FILE="$PROJECT_ROOT/.swt-planning/.execution-state.json"
EXEC_ACTIVE=false
...
if is_teammate_spawn_tool; then
  # Non-team .tool_input.name is platform label metadata; SWT routing and
  # lifecycle state must come from delegation markers and team_name only.
  # Hard blocks first — SWT-internal invariants must reject before strip paths
  if requested_vbw_worktree_cwd; then
    echo "Blocked: teammate spawn requested a SWT worktree path as a spawn working directory. Omit cwd/working_dir/workingDirectory/workdir fields; SWT worktree targeting is task prompt/state metadata, not a spawn cwd." >&2
    exit 2
  fi
  ...
  if requested_sidechain_cwd; then
    ...
    STRIP_REASON="SWT stripped sidechain cwd fields — worktree targeting is task metadata, not spawn cwd"
    STRIP_WARN="SWT guard: stripped sidechain cwd fields (and isolation if present) from $TOOL_NAME spawn (models add these spontaneously; blocking causes infinite retry loops)"
  elif requested_worktree_isolation; then
    STRIPPED_INPUT=$(echo "$INPUT" | jq '.tool_input | del(.isolation)' 2>/dev/null)
    STRIP_REASON="SWT stripped isolation:worktree — worktree isolation is not managed via spawn params"
    STRIP_WARN="SWT guard: stripped isolation:worktree from $TOOL_NAME spawn (models add this spontaneously; blocking causes infinite retry loops)"
  fi
fi
  ...
  subagent|direct)
    ...
    if [ "$TOOL_NAME" = "TaskCreate" ]; then
      ...
      if command -v vbw_active_agent_current_count >/dev/null 2>&1; then
        ACTIVE_COUNT=$(vbw_active_agent_current_count "$PROJECT_ROOT/.swt-planning" "$INPUT")
      elif [ -f "$PROJECT_ROOT/.swt-planning/.active-agent-count" ]; then
        ACTIVE_COUNT=$(cat "$PROJECT_ROOT/.swt-planning/.active-agent-count" 2>/dev/null | tr -d '[:space:]')
      fi
