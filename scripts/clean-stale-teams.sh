...
# Pass 2: Clean stale SWT teams (have config.json but inbox older than threshold)
# Same swt-* scope as pass 1 — SWT should not remove other plugins' teams.
for team_dir in "$TEAMS_DIR"/*; do
  [ ! -d "$team_dir" ] && continue
