...
artifact_phase_dir() {
  ...
  root="${path%%/.swt-planning/phases/*}"
  suffix="${path#*/.swt-planning/phases/}"
  phase_slug="${suffix%%/*}"
...
validate_legacy_phase_root_path() {
  ...
  phase_dir=$(artifact_phase_dir "$ARTIFACT_PATH") \
    || emit_failure "artifact path must point at an active .swt-planning/phases/ artifact"
...
validate_common_path() {
  ...
  case "$ARTIFACT_PATH" in
    */.swt-planning/phases/*/remediation/uat/round-[0-9]*/*)
      REMEDIATION_KIND="uat"
      validate_round_dir_artifact_path "uat" "$expected_suffix"
      ;;
    */.swt-planning/phases/*/remediation/qa/round-[0-9]*/*)
      REMEDIATION_KIND="qa"
      ...
      ;;
    */.swt-planning/phases/*/remediation/qa/*)
