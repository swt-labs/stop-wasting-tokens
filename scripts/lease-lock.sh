...
# Phase 2 §6.3 path-only rewrite (plan 02-03).
#
# Lease-based file locks with TTL + heartbeat. Four canonical verbs:
#   acquire | renew | release | check   (plus query: read-only inspection)
#
# The vendor-neutral mkdir-based approach is intentional — do NOT "upgrade"
# it to flock or a fancier primitive; it is portable across macOS + Linux and
# does not depend on Claude Code primitives.
#
# Lock files: ${SWT_PLANNING_DIR}/.locks/{task-id}.lock as JSON with the
# canonical schema (preserved bit-for-bit so the orchestrator's TypeScript
# implementation at packages/orchestration/src/lock-files.ts can also consume
# it):
#   { task_id, pid, timestamp, files[], ttl, expires_at }
#
# The TypeScript lock-files.ts and this bash script are parallel mechanisms
# targeting different callers (TS orchestrator vs. bash control plane).
# Phase 6 reconciles them (Research Risk 7); Phase 2 leaves both intact.
#
# Fail-open: exit 0 on every path except a true conflict when
# v2_hard_gates is true (where exit 1 is intentional — REQ-04).
