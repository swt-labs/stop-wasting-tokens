#!/bin/bash
set -u
# UserPromptSubmit: Pre-flight validation for SWT commands (non-blocking, exit 0)
...
is_archive_vibe_prompt() {
  ...
  if printf '%s\n' "$prompt" | grep -qiE 'swt cook[^[:cntrl:]]*--archive'; then
    return 0
  fi
...
# SWT context marker: signal statusline that SWT context is active in this session.
# Created on any SWT command; cleared by session-start.sh (new session) and
# post-compact.sh (compaction degrades context reliability).
if echo "$PROMPT" | grep -qi '^swt ' || is_expanded_vbw_prompt "$PROMPT"; then
  echo "1" > "$PLANNING_DIR/.swt-context" 2>/dev/null || true
fi

# GSD Isolation: create .swt-session marker on SWT command invocation.
# Detection covers raw slash commands (swt *) and expanded command content
# (YAML frontmatter with "name: vbw:").
# Only CREATE the marker here; removal is handled by session-stop.sh at session end.
# Deleting on non-swt  prompts caused false blocks mid-workflow when users send
# follow-up messages (plan approvals, answers) that don't start with swt .
if [ -f "$PLANNING_DIR/.gsd-isolation" ]; then
  if echo "$PROMPT" | grep -qi '^swt '; then
    echo "session" > "$PLANNING_DIR/.swt-session"
...
# Check: swt cook --execute when no PLAN.md exists
if echo "$PROMPT" | grep -q 'swt cook.*--execute'; then
  CURRENT_PHASE=""
  ...
  if [ -n "$CURRENT_PHASE" ]; then
    ...
    if [ "$PLAN_COUNT" -eq 0 ]; then
      WARNING="No PLAN.md for phase $CURRENT_PHASE. Run swt cook to plan first."
    fi
  fi
fi

# Check: swt cook --archive with incomplete phases
...
if [ -n "$BLOCK_MSG" ]; then
  jq -n --arg msg "$BLOCK_MSG" '{
    "hookSpecificOutput": {
      ...
      "additionalContext": ("SWT pre-flight block: " + $msg)
    }
  }'
  exit 2
fi
...
if [ -n "$WARNING" ]; then
  jq -n --arg msg "$WARNING" '{
    "hookSpecificOutput": {
      "hookEventName": "UserPromptSubmit",
      "additionalContext": ("SWT pre-flight warning: " + $msg)
    }
  }'
fi
