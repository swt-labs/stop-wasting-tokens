...
if [ -f "$_FG_SCRIPT_DIR/lib/swt-config-root.sh" ]; then
  # shellcheck source=lib/swt-config-root.sh
  if source "$_FG_SCRIPT_DIR/lib/swt-config-root.sh" 2>/dev/null; then
...
find_project_root() {
  local dir="$PWD"
  while [ "$dir" != "/" ]; do
    if [ -f "$dir/.swt-planning/config.json" ] || [ -d "$dir/.swt-planning/phases" ]; then
      echo "$dir"
      return 0
    fi
    dir=$(dirname "$dir")
...
if [ "$_FG_SHARED_ROOT_RESOLVED" = "true" ] && [ -n "${VBW_CONFIG_ROOT:-}" ] && [ -n "${VBW_PLANNING_DIR:-}" ] && [ -f "$VBW_PLANNING_DIR/config.json" ]; then
  ...
  if [ -n "$PROJECT_ROOT" ]; then
    PHASES_DIR="$PROJECT_ROOT/.swt-planning/phases"
  else
    PHASES_DIR=""
  fi
fi
...
normalize_agent_role() {
  ...
  case "$lower" in
    swt-scout|swt-scout-[0-9]*|scout|scout-[0-9]*|team-scout|team-scout-[0-9]*)
      printf 'scout'
      return 0
      ;;
    swt-lead|swt-lead-[0-9]*|lead|lead-[0-9]*|team-lead|team-lead-[0-9]*)
      printf 'lead'
      return 0
      ;;
    swt-dev|swt-dev-[0-9]*|dev|dev-[0-9]*|team-dev|team-dev-[0-9]*)
      printf 'dev'
      return 0
      ;;
    swt-qa|swt-qa-[0-9]*|qa|qa-[0-9]*|team-qa|team-qa-[0-9]*)
      printf 'qa'
      return 0
      ;;
    swt-debugger|swt-debugger-[0-9]*|debugger|debugger-[0-9]*|team-debugger|team-debugger-[0-9]*)
      printf 'debugger'
      return 0
      ;;
    swt-architect|swt-architect-[0-9]*|architect|architect-[0-9]*|team-architect|team-architect-[0-9]*)
      printf 'architect'
      return 0
      ;;
    swt-docs|swt-docs-[0-9]*|docs|docs-[0-9]*|team-docs|team-docs-[0-9]*)
      printf 'docs'
      return 0
      ;;
  esac
...
detect_agent_role() {
  ...
  if [ -n "$PROJECT_ROOT" ]; then
    planning_dir="$PROJECT_ROOT/.swt-planning"
...
# Must precede the .swt-planning/* exemption which exits 0 for all planning artifacts.
...
case "$FILE_PATH_LC" in
  *.swt-planning/phases/*/plan-[0-9]*.md|*.swt-planning/phases/*/summary-[0-9]*.md|*.swt-planning/phases/*/context-[0-9]*.md)
...
# writes under the sidechain, would land in files SWT does not merge/use when
# SWT worktree isolation is off. Block those targets before planning-artifact
  ...
  if [ "$_FG_SIDECHAIN_BLOCK" = "true" ]; then
    {
      ...
      echo "reason: SWT will not merge or use writes made inside Claude's internal sidechain."
    } >&2
    exit 2
  fi
fi
...
# PreToolUse payload may not include SWT_AGENT_ROLE, so fall back to current
...
if [ "$ACTIVE_AGENT_ROLE" = "scout" ] && [ -n "$PROJECT_ROOT" ]; then
  _FG_TARGET_ABS=$(to_abs_path "$FILE_PATH")
  _FG_PLANNING_ABS=$(to_abs_path "$PROJECT_ROOT/.swt-planning")
  case "$_FG_TARGET_ABS" in
    "$_FG_PLANNING_ABS"|"$_FG_PLANNING_ABS"/*)
      :
      ;;
    *)
      echo "Blocked: Scout-safe active-agent context is read-only outside .swt-planning/" >&2
      exit 2
      ;;
  esac
fi
...
case "$FILE_PATH" in
  *.swt-planning/milestones/*/phases/*)
    ...
    # general .swt-planning/* exemption below) because Archive mode
    ...
    exit 2
    ;;
  *.swt-planning/*/remediation/uat/round-*/R[0-9]*-SUMMARY.md|\
  *.swt-planning/*/remediation/qa/round-*/R[0-9]*-SUMMARY.md)
  ...
  *.swt-planning/*-SUMMARY.md)
    ...
    exit 0
    ;;
  *.swt-planning/*|*SUMMARY.md|*VERIFICATION.md|*STATE.md|*CLAUDE.md|*.execution-state.json)
    exit 0
    ;;
esac
...
CONFIG_PATH="$PROJECT_ROOT/.swt-planning/config.json"
...
if [ "$WORKTREE_ISOLATION" != "off" ] && [ -n "${VBW_AGENT_ROLE:-}" ]; then
  case "${VBW_AGENT_ROLE:-}" in
    dev|debugger)
      AGENT_NAME_SHORT=$(echo "${VBW_AGENT_NAME:-}" | sed 's/.*swt-//')
      WORKTREE_MAP_FILE="$PROJECT_ROOT/.swt-planning/.agent-worktrees/${AGENT_NAME_SHORT}.json"
...
if true; then
  CONTRACT_DIR="$PROJECT_ROOT/.swt-planning/.contracts"
...
# When a SWT delegated workflow is active (execute, fix, debug) and the caller is
# the orchestrator (no SWT_AGENT_ROLE), block product-file writes. The orchestrator
...
#
# NOTE: SWT_AGENT_ROLE is NOT set by the Claude Code runtime for PreToolUse hooks.
# Subagent detection uses .active-agent-count (written by agent-start.sh, decremented
# by agent-stop.sh). If count > 0, at least one SWT subagent is running and this
...
# hooks, so `.active-agent-count` alone is insufficient to detect them. SWT
...
if [ -z "${VBW_AGENT_ROLE:-}" ]; then
  # Check active-agent count: if a SWT subagent is active in the current safe
  ...
  if command -v vbw_active_agent_current_count >/dev/null 2>&1; then
    _DG_AGENT_COUNT=$(vbw_active_agent_current_count "$PROJECT_ROOT/.swt-planning" "$INPUT")
  elif [ -f "$PROJECT_ROOT/.swt-planning/.active-agent-count" ]; then
    _DG_AGENT_COUNT=$(cat "$PROJECT_ROOT/.swt-planning/.active-agent-count" 2>/dev/null | tr -d '[:space:]')
  fi
  if echo "$_DG_AGENT_COUNT" | grep -Eq '^[0-9]+$' && [ "$_DG_AGENT_COUNT" -gt 0 ]; then
    # SWT subagent is active in this session — allow the write
    exit 0
  fi

  _DELEG_FILE="$PROJECT_ROOT/.swt-planning/.delegated-workflow.json"
  _DG_MARKER_STATUS=""
  _DG_MARKER_LIVE="false"
  _DG_MARKER_MODE=""
  _DG_MARKER_EXEC_MODE=""
  if [ -f "$_DELEG_FILE" ]; then
    _DG_MARKER_STATUS=$(VBW_PLANNING_DIR="$PROJECT_ROOT/.swt-planning" bash "${_FG_SCRIPT_DIR}/delegated-workflow.sh" status-json 2>/dev/null) || _DG_MARKER_STATUS=""
  ...
  _EXEC_STATE_FILE="$PROJECT_ROOT/.swt-planning/.execution-state.json"
  ...
  if [ "$_DG_BLOCK" = true ]; then
    ...
    if [ -z "$_DG_EFFORT" ] || [ "$_DG_EFFORT" = "null" ]; then
      _DG_EFFORT=$(jq -r '.effort // "balanced"' "$PROJECT_ROOT/.swt-planning/config.json" 2>/dev/null) || _DG_EFFORT="balanced"
    fi
    case "$_DG_EFFORT" in
      turbo|direct)
...
if [ -n "$AGENT_ROLE" ]; then
  case "$AGENT_ROLE" in
    lead|architect|qa)
      # Planning roles can only write to .swt-planning/ (already exempted above, so reaching here means non-planning path)
      echo "Blocked: role '${AGENT_ROLE}' cannot write outside .swt-planning/" >&2
      exit 2
      ;;
    scout)
