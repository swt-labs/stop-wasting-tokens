...
if [ -f "$SCRIPT_DIR/lib/swt-config-root.sh" ]; then
  # shellcheck source=scripts/lib/swt-config-root.sh
  source "$SCRIPT_DIR/lib/swt-config-root.sh"
fi
...
reject_milestone_phase_dir() {
  ...
  case "$phase_dir" in
    */.swt-planning/milestones/*|.swt-planning/milestones/*)
      echo "Error: refusing to operate on archived milestone path: $phase_dir" >&2
      echo "QA remediation must target active phases in .swt-planning/phases/" >&2
      exit 1
      ;;
  esac
}
...
translate_claude_sidechain_phase_candidate() {
  ...
  # Primary: env-var mapping populated by swt-config-root.sh when the shell CWD is inside a
  # Claude sidechain (e.g. .claude/worktrees/agent-*).
  if [ -n "$sidechain_root" ] && [ -n "$host_root" ] && [ -f "$host_root/.swt-planning/config.json" ]; then
    case "$candidate" in
      "$sidechain_root"/.swt-planning/phases/*)
        rel_phase_path="${candidate#"$sidechain_root"/.swt-planning/phases/}"
        printf '%s/.swt-planning/phases/%s\n' "$host_root" "$rel_phase_path"
        return 0
        ;;
    esac
  fi
  ...
  case "$candidate" in
    */.claude/worktrees/agent-*/.swt-planning/phases/*)
      inferred_sidechain_root="${candidate%%/.swt-planning/phases/*}"
      rel_phase_path="${candidate#"$inferred_sidechain_root"/.swt-planning/phases/}"
      ...
      if [ "$(basename "$worktrees_dir")" = "worktrees" ] \
        ...
        && [ -f "$inferred_host_root/.swt-planning/config.json" ]; then
...
normalize_active_phase_dir_root() {
  ...
  case "$candidate" in
    */.swt-planning/phases/*)
      phase_root="${candidate%%/.swt-planning/phases/*}"
      suffix="${candidate#*/.swt-planning/phases/}"
      phase_slug="${suffix%%/*}"
...
canonicalize_phase_dir() {
  ...
  case "$candidate" in
    */.swt-planning/phases/*) ;;
    *)
      echo "Error: QA remediation phase must be under active .swt-planning/phases/: $candidate" >&2
  ...
  if [ -z "$phase_root" ] || [ "$phase_root" = "$candidate" ]; then
    echo "Error: unable to determine SWT root from phase path: $candidate" >&2
    exit 1
  fi

  export SWT_CONFIG_ROOT="$phase_root"
  export SWT_PLANNING_DIR="$phase_root/.swt-planning"
  printf '%s\n' "$candidate"
}
