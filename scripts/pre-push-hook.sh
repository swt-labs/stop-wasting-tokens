...
# This hook is SWT-specific. Skip entirely if bump-version.sh is absent.
# Note: checking VERSION alone is insufficient — many non-SWT repos have VERSION files.
if [ ! -f "$ROOT/scripts/bump-version.sh" ]; then
  exit 0
fi

# Plugin-name guard (issue #635): mirror validate-commit.sh's check so we never
# invoke `--verify` against a non-SWT bump-version.sh. Many repos have their own
...
# field. Only proceed when this is a SWT-managed repo (the same condition
# validate-commit.sh already enforces). Non-SWT repos can either name their
