...
# swt-target-root.sh — Shared target root resolution for context scripts.
#
...
vbw_walk_up_for_workspace_root() {
  ...
  while :; do
    if [ "$(basename "$current")" = ".swt-planning" ]; then
      dirname "$current"
      return 0
    fi

    if [ -d "$current/.swt-planning" ]; then
      printf '%s\n' "$current"
      return 0
    fi
...
vbw_resolve_target_planning_dir() {
  ...
  for candidate in "$@"; do
    ...
    if [ "$(basename "$candidate_dir")" = ".swt-planning" ]; then
      printf '%s\n' "$candidate_dir"
      return 0
    fi
  done
  ...
  if [ -n "$target_root" ] && [ -d "$target_root/.swt-planning" ]; then
    printf '%s/.swt-planning\n' "$target_root"
    return 0
  fi
