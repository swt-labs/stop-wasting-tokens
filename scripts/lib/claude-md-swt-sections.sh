...
# Shared helpers for generating and detecting SWT-managed CLAUDE.md sections.
...
**Next action:** Run swt cook to start a new milestone, or swt status to review progress
EOF
}
...
- **Plan before building.** Use swt cook for all lifecycle actions. Plans are the source of truth.
- **Do not fabricate content.** Only use what the user explicitly states in project-defining flows.
- **Do not bump version or push until asked.** Never run `scripts/bump-version.sh` or `git push` unless the user explicitly requests it, except when `.swt-planning/config.json` intentionally sets `auto_push` to `always` or `after_phase`.
EOF
}
...
- When both plugins are active, treat each plugin's context as separate. Do not mix GSD project insights into SWT planning or vice versa.
EOF
}
  ...
  awk '
    function should_remove_section(    remove_section) {
      ...
      if (section_header == "## Active Context" ||
          section_header == "## SWT Rules" ||
          section_header == "## Code Intelligence" ||
          section_header == "## Plugin Isolation") {
        remove_section = 1
      } else if (section_header == "## State") {
        if (index(section_body, "Planning directory: `.swt-planning/`") > 0) {
          remove_section = 1
        }
      } else if (section_header == "## Project Conventions") {
        if (index(section_body, "None yet. Run swt teach to add project conventions.") > 0 ||
        ...
        if (index(section_body, "Run swt status for current progress.") > 0 &&
            index(section_body, "Run swt help for all available commands.") > 0) {
          remove_section = 1
        }
      }
