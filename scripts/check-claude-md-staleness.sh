...
# check-claude-md-staleness.sh — Detect and fix stale SWT sections in CLAUDE.md
#
# Usage:
#   check-claude-md-staleness.sh [--fix] [--json]
#
# Checks whether CLAUDE.md has all SWT-managed sections and whether the
# version marker (.swt-planning/.claude-md-version) matches the installed
# plugin version. Outputs human-readable text by default, JSON with --json.
#
# With --fix: re-runs bootstrap-claude.sh to regenerate SWT sections while
...
LIB="$SCRIPT_DIR/lib/claude-md-swt-sections.sh"
...
if [ "$FIX" = true ] && [ "$STALE" = true ]; then
  REFRESHER="$SCRIPT_DIR/refresh-claude-md-swt-sections.sh"
  if [ ! -f "$REFRESHER" ]; then
    echo "ERROR: refresh-claude-md-swt-sections.sh not found at $REFRESHER" >&2
    exit 2
  fi

  # Refresh only SWT-owned sections in-place; do not rebuild arbitrary user content.
  if bash "$REFRESHER" "$CLAUDE_MD" 2>/dev/null; then
    ...
    if [ "$JSON" = true ]; then
      ...
      echo "CLAUDE.md SWT sections refreshed in place (${MARKER_VER:-none} -> ${INSTALLED_VER:-unknown}). Non-SWT content preserved."
    fi
    exit 0
  else
    echo "ERROR: refresh-claude-md-swt-sections.sh failed" >&2
    exit 2
  fi
fi
...
if [ "$STALE" = true ]; then
  echo "STALE: CLAUDE.md SWT sections need refresh"
  if [ ${#MISSING_SECTIONS[@]} -gt 0 ]; then
  ...
  echo "  Run with --fix to refresh SWT-owned sections in place (all other CLAUDE.md content preserved)."
  exit 1
else
  ...
  echo "FRESH: CLAUDE.md SWT sections are current (v${INSTALLED_VER:-unknown})"
  exit 0
fi
