...
if [ -z "${VBW_PLANNING_DIR:-}" ] && [ -f "$SCRIPT_DIR/lib/swt-config-root.sh" ]; then
  # shellcheck source=lib/swt-config-root.sh
  if source "$SCRIPT_DIR/lib/swt-config-root.sh" 2>/dev/null; then
    find_swt_root "$SCRIPT_DIR" >/dev/null 2>&1 || true
  fi
fi
...
if echo "$COMMAND" | grep -iqE "$PATTERNS"; then
  ...
  echo "Hint: Use SWT_ALLOW_DESTRUCTIVE=1 to override, or run outside SWT." >&2
