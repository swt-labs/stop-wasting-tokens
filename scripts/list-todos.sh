...
resolve_state_path() {
  ...
  echo '{"status":"error","message":"STATE.md not found at '"$state_path"'. Run swt init to set up your project."}'
  return 1
}
...
main() {
  ...
  all_items_ndjson=$(mktemp 2>/dev/null) || {
    emit_error_json "$state_path" "$section_name" "$filter_lower" "Could not prepare todo listing output. Rerun swt list-todos."
    exit 0
  }
  while IFS= read -r line; do
    ...
    if ! parsed=$(todo_item_parse_line_json "$line" "$section_index" "$state_path" "$section_name"); then
      ...
      emit_error_json "$state_path" "$section_name" "$filter_lower" "Could not parse one of the current todos. Rerun swt list-todos."
      exit 0
    fi
    if ! printf '%s\n' "$parsed" >> "$all_items_ndjson"; then
      rm -f "$all_items_ndjson"
      emit_error_json "$state_path" "$section_name" "$filter_lower" "Could not stage todo listing output. Rerun swt list-todos."
      exit 0
    fi
  done <<< "$todo_lines"
  ...
  all_items_json=$(jq -sc '.' "$all_items_ndjson" 2>/dev/null) || {
    rm -f "$all_items_ndjson"
    emit_error_json "$state_path" "$section_name" "$filter_lower" "Could not build the todo listing. Rerun swt list-todos."
    exit 0
  }
  rm -f "$all_items_ndjson"
  all_items_json=$(todo_item_annotate_identity_occurrence "$all_items_json") || {
    emit_error_json "$state_path" "$section_name" "$filter_lower" "Could not annotate todo identity metadata. Rerun swt list-todos."
    exit 0
  }
  items_json=$(printf '%s' "$all_items_json" | jq -c --arg filter "$filter_lower" '
    ...
    | map(.value + {num:(.key + 1)})
  ' 2>/dev/null) || {
    emit_error_json "$state_path" "$section_name" "$filter_lower" "Could not filter todo items. Rerun swt list-todos."
    exit 0
  }

  local filtered_count
  filtered_count=$(echo "$items_json" | jq 'length' 2>/dev/null) || {
    emit_error_json "$state_path" "$section_name" "$filter_lower" "Could not count todo items. Rerun swt list-todos."
    exit 0
  }
  ...
  display=$(printf '%s' "$items_json" | jq -r '
    .[] |
      ...
      | "\(.num). \($body)"
  ' 2>/dev/null) || {
    emit_error_json "$state_path" "$section_name" "$filter_lower" "Could not render the todo listing. Rerun swt list-todos."
    exit 0
  }
  ...
  echo "$items_json" | jq --arg st "ok" --arg sp "$state_path" \
    ...
    emit_error_json "$state_path" "$section_name" "$filter_lower" "Could not finalize the todo listing. Rerun swt list-todos."
    exit 0
  }
}
