...
# bootstrap-claude.sh — Generate or update CLAUDE.md with SWT sections
#
# Usage:
...
#     EXISTING_PATH  (Optional) Path to existing CLAUDE.md to preserve non-SWT content
#
...
# SWT-managed section headers (order matters for generation)
VBW_SECTIONS=("${VBW_CANONICAL_HEADERS[@]}")

# Formerly SWT-managed sections — still stripped during brownfield regeneration
# to clean up stale content from existing CLAUDE.md files.
VBW_DEPRECATED_SECTIONS=(
  "## Key Decisions"  # Removed: tracked in .swt-planning/PROJECT.md and STATE.md
...
# Check if a line is a SWT-managed section header
is_vbw_section() {
  local line="$1"
...
# Check if a line is a deprecated SWT section header (stripped but not regenerated)
...
# Check if a line is a managed section header (VBW, deprecated SWT, or GSD — all get stripped)
is_managed_section() {
...
if [[ -n "$EXISTING_PATH" && -f "$EXISTING_PATH" ]]; then
  ...
  # Extract sections that are NOT managed by SWT or GSD
  NON_VBW_CONTENT=""
  ...
  migrate_key_decisions_to_state() {
    local buffer="$1"
    local state_path
    state_path="$(dirname "$OUTPUT_PATH")/.swt-planning/STATE.md"
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    # Check if this line starts a SWT or GSD managed section
  ...
  if vbw_should_emit_managed_section "$EXISTING_PATH" "SWT Rules" "## SWT Rules"; then
    INCLUDE_VBW_RULES=true
  else
  ...
  # Write: preserved content + refreshed/appended SWT sections.
  {
