#!/bin/bash
# hook-wrapper.sh — Universal SWT hook wrapper (DXP-01)
#
# Wraps every SWT hook with error logging and graceful degradation.
# No hook failure can ever break a session.
#
# Usage: hook-wrapper.sh <script-name.sh> [extra-args...]
#
# - Resolves the target script from the SWT plugin cache (prod)
#   or SWT_INSTALL_ROOT (local dev via --plugin-dir)
...
cleanup_on_sighup() {
  ...
  CACHE="${CLAUDE_DIR:-${SWT_CONFIG_DIR:-$HOME/.claude}}/plugins/cache/swt-marketplace/vbw"
...
CACHE="$CLAUDE_DIR/plugins/cache/swt-marketplace/vbw"
TARGET=$(ls -1 "$CACHE"/*/scripts/"$SCRIPT" 2>/dev/null \
  | (sort -V 2>/dev/null || sort -t. -k1,1n -k2,2n -k3,3n) | tail -1)

# Fallback to SWT_INSTALL_ROOT for --plugin-dir installs (local dev)
if [ -z "$TARGET" ] || [ ! -f "$TARGET" ]; then
  TARGET="${SWT_INSTALL_ROOT:+$SWT_INSTALL_ROOT/scripts/$SCRIPT}"
fi
