/**
 * Cassette scenario: qa-against-ref-fastapi.
 *
 * Drives the SWT qa agent against the frozen
 ...
 *
 * Re-recording: `pnpm record -- --scenario=qa-against-ref-fastapi`.
 */
...
export async function run({ provider, model, apiKey }) {
  if (!apiKey) {
    throw new Error(
      'qa-against-ref-fastapi: missing apiKey. Set ANTHROPIC_API_KEY before `pnpm record`.',
    );
  }

  const tmpRoot = mkdtempSync(join(tmpdir(), 'swt-rec-qa-ref-fastapi-'));
  cpSync(SPEC_DIR, tmpRoot, { recursive: true });
  process.env['NODE_ENV'] = 'test';
  process.env['SWT_DEBUG_ONLY_ROLE'] = 'qa';
