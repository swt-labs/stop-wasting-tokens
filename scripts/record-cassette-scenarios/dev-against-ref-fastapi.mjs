/**
 * Cassette scenario: dev-against-ref-fastapi.
 *
 * Drives the SWT dev agent against the frozen
 ...
 *
 * Re-recording: `pnpm record -- --scenario=dev-against-ref-fastapi`.
 */
...
export async function run({ provider, model, apiKey }) {
  if (!apiKey) {
    throw new Error(
      'dev-against-ref-fastapi: missing apiKey. Set ANTHROPIC_API_KEY before `pnpm record`.',
    );
  }

  const tmpRoot = mkdtempSync(join(tmpdir(), 'swt-rec-dev-ref-fastapi-'));
  cpSync(SPEC_DIR, tmpRoot, { recursive: true });
  process.env['NODE_ENV'] = 'test';
  process.env['SWT_DEBUG_ONLY_ROLE'] = 'dev';
