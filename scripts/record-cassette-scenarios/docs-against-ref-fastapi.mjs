/**
 * Cassette scenario: docs-against-ref-fastapi.
 *
 * Drives the SWT docs agent against the frozen
 ...
 *
 * Re-recording: `pnpm record -- --scenario=docs-against-ref-fastapi`.
 */
...
export async function run({ provider, model, apiKey }) {
  if (!apiKey) {
    throw new Error(
      'docs-against-ref-fastapi: missing apiKey. Set ANTHROPIC_API_KEY before `pnpm record`.',
    );
  }

  const tmpRoot = mkdtempSync(join(tmpdir(), 'swt-rec-docs-ref-fastapi-'));
  cpSync(SPEC_DIR, tmpRoot, { recursive: true });
  process.env['NODE_ENV'] = 'test';
  process.env['SWT_DEBUG_ONLY_ROLE'] = 'docs';
