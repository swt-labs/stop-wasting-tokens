/**
 * Cassette scenario: debugger-against-ref-fastapi.
 *
 * Drives the SWT debugger agent against the frozen
 ...
 *
 * Re-recording: `pnpm record -- --scenario=debugger-against-ref-fastapi`.
 */
...
export async function run({ provider, model, apiKey }) {
  if (!apiKey) {
    throw new Error(
      'debugger-against-ref-fastapi: missing apiKey. Set ANTHROPIC_API_KEY before `pnpm record`.',
    );
  }

  const tmpRoot = mkdtempSync(join(tmpdir(), 'swt-rec-debugger-ref-fastapi-'));
