...
# from .swt-planning/), converts bytes to tokens (~5 chars/token for
...
#
# Reads:
#   .swt-planning/.context-usage   — statusline cache written as
...
#   .swt-planning/config.json      — compaction_threshold, autonomy, effort
#   Plugin root reference files     — measured per mode
#   .swt-planning/ project files    — measured per mode
#
# Output (stdout):
...
PLUGIN_ROOT="${SWT_INSTALL_ROOT:-}"
if [ -z "$PLUGIN_ROOT" ]; then
  PLUGIN_ROOT=$(find "${CLAUDE_DIR:-${SWT_CONFIG_DIR:-$HOME/.claude}}/plugins/cache/swt-marketplace/vbw" -maxdepth 1 -mindepth 1 -type d 2>/dev/null | (sort -V 2>/dev/null || sort -t. -k1,1n -k2,2n -k3,3n) | tail -1 || true)
fi
...
case "$MODE" in
  execute)
    ...
    FIXED_BYTES=$(sum_bytes \
      ...
      "$PLUGIN_ROOT/references/swt-brand-essentials.md" \
      "$(effort_file)" \
      "$PLUGIN_ROOT/agents/swt-dev.md" \
      "$PLUGIN_ROOT/agents/swt-qa.md" \
  ...
  plan)
    ...
    FIXED_BYTES=$(sum_bytes \
      "$PLUGIN_ROOT/agents/swt-lead.md" \
...
# `unknown` is a writer-side placeholder from swt-statusline.sh, not proof that
...
**RECOMMENDED:** Run \`/compact\` first, then re-run this command. Or run \`swt pause\` then \`swt resume\` for a clean context reload.
EOF
  fi
fi
