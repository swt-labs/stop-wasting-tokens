...
# SWT Local Development Setup
# Sets up (or tears down) the local dev environment for contributing to SWT.
#
# Usage:
...
#
# Setup must be run from the root of the SWT clone.
# --teardown and --status work from any directory.

# ---------- constants ----------

CLAUDE_DIR="${SWT_CONFIG_DIR:-$HOME/.claude}"
CACHE_BASE="$CLAUDE_DIR/plugins/cache/swt-marketplace/vbw"
...
check_clone_root() {
  if [ ! -f "plugin.json" ] && [ ! -f ".claude-plugin/plugin.json" ]; then
    err "Not in the SWT repo root. Run this from the cloned directory."
...
do_status() {
  step "SWT dev environment status"
  ...
  if [ -n "$repo_root" ] && [ -f "$repo_root/.git/hooks/pre-push" ] && grep -q "SWT pre-push hook" "$repo_root/.git/hooks/pre-push" 2>/dev/null; then
...
do_setup() {
  ...
  # Only targets direct children of the SWT-specific cache path, so this
  ...
  if [ -f "$launcher_script" ]; then
    ...
    printf 'Install claude-vbw command to launch Claude Code with local SWT from anywhere? [y/N] '
  ...
  echo ""
  echo "Launch Claude Code with your local SWT:"
  echo ""
  if [ -L "$LAUNCHER_LINK" ]; then
    echo "  claude-vbw                             # from anywhere (uses local SWT)"
  fi
  echo "  claude --plugin-dir .                  # from this repo"
  echo "  claude --plugin-dir $clone   # explicit absolute path"
  echo ""
  echo "All swt * commands will load from your local copy."
  echo "Restart Claude Code to pick up changes after editing SWT files."
  echo ""
...
do_teardown() {
  ...
  if [ -n "$repo_root" ] && [ -f "$repo_root/.git/hooks/pre-push" ] && grep -q "SWT pre-push hook" "$repo_root/.git/hooks/pre-push" 2>/dev/null; then
    ...
    ok "Removed SWT pre-push hook"
  else
    ok "No SWT pre-push hook to remove"
  fi
  ...
  echo ""
  echo "To reinstall SWT from the marketplace, start Claude Code and run:"
  echo ""
  echo "  /plugin marketplace add yidakee/vibe-better-with-claude-code-vbw"
  echo "  /plugin install vbw@swt-marketplace"
  echo ""
}
...
case "${1:-}" in
  ...
  -h|--help)
    ...
    echo "  --teardown   Revert to marketplace SWT"
