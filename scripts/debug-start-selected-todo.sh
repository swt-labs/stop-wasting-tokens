...
# debug-start-selected-todo.sh — deterministic swt debug numbered-todo startup.
#
# Usage:
#   debug-start-selected-todo.sh <planning-dir> <N> [--competing|--parallel|--serial]
#
# Resolves a numbered todo from the persisted unfiltered swt list-todos snapshot,
...
ARCHIVED_STATE_MESSAGE="This todo came from archived milestone state. Restore the writable root STATE.md first by restarting so session-start.sh can run migration, or run 'bash scripts/migrate-orphaned-state.sh .swt-planning'."
...
canonicalize_debug_sessions() {
  SWT_PLANNING_DIR="$PLANNING_DIR" bash "$SCRIPT_DIR/debug-session-state.sh" list "$PLANNING_DIR" >/dev/null 2>&1 || true
}
...
pickup_selected_todo() {
  ...
  SWT_PLANNING_DIR="$PLANNING_DIR" printf '%s' "$selected_json" | \
    SWT_PLANNING_DIR="$PLANNING_DIR" bash "$SCRIPT_DIR/todo-lifecycle.sh" pickup swt debug "$detail_status" "$cleanup_policy" 2>/dev/null || true
}
...
while [ $# -gt 0 ]; do
  case "$1" in
    --competing|--parallel|--serial)
      ...
      ;;
    *)
      json_error "usage" "Unknown routing flag for swt debug selected todo: $1"
      exit 0
      ;;
  esac
  shift || true
...
if ! [[ "$SELECTION" =~ ^[0-9]+$ ]]; then
  json_error "usage" "Invalid todo selection — choose a numbered todo from swt list-todos."
  exit 0
fi
...
cd "$PROJECT_ROOT"
export SWT_PLANNING_DIR="$PLANNING_DIR"
...
if ! json_is_valid "$SELECTED_JSON"; then
  json_error "resolver_invalid_json" "Todo selection resolver returned invalid JSON. Rerun swt list-todos."
  exit 0
fi
...
SESSION_OUTPUT=""
if [ "$DETAIL_STATUS" = "ok" ]; then
  SESSION_OUTPUT=$(printf '%s' "$SELECTED_JSON" | TODO_DETAIL_RESULT_JSON="$DETAIL_RESULT_JSON" SWT_PLANNING_DIR="$PLANNING_DIR" bash "$SCRIPT_DIR/debug-session-state.sh" start-with-selected-todo "$PLANNING_DIR" "$SLUG" "$DETAIL_STATUS" 2>&1) || {
    json_error "session_start_failed" "$SESSION_OUTPUT"
    exit 0
  }
else
  SESSION_OUTPUT=$(printf '%s' "$SELECTED_JSON" | SWT_PLANNING_DIR="$PLANNING_DIR" bash "$SCRIPT_DIR/debug-session-state.sh" start-with-selected-todo "$PLANNING_DIR" "$SLUG" "$DETAIL_STATUS" 2>&1) || {
    json_error "session_start_failed" "$SESSION_OUTPUT"
    exit 0
  }
fi
