...
awk -f "$ISSUE_PARSER" "$UAT_FILE" > /tmp/.swt-uat-issues-$$.txt
trap 'rm -f /tmp/.swt-uat-issues-$$.txt /tmp/.swt-uat-round-ids-$$.txt' EXIT

ISSUE_COUNT=$(wc -l < /tmp/.swt-uat-issues-$$.txt | tr -d ' ')
...
if [ "$MAX_ARCHIVED" -gt 0 ]; then
  ...
  done >> /tmp/.swt-uat-round-ids-$$.txt
...
while IFS='|' read -r id severity desc; do
  ...
  PAST_ROUNDS=""
  if [ -s /tmp/.swt-uat-round-ids-$$.txt ]; then
    PAST_ROUNDS=$(grep "^${id} " /tmp/.swt-uat-round-ids-$$.txt | awk '{print $2}' | sort -n | tr '\n' ',' | sed 's/,$//' || true)
  fi
