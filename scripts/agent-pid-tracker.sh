#!/bin/bash
set -euo pipefail
# agent-pid-tracker.sh — Track active SWT agent PIDs for cleanup on tmux detach
#
# Usage:
...
#
# Stores newline-delimited PIDs in .swt-planning/.agent-pids
...
LOCK_DIR="${VBW_AGENT_PID_LOCK_DIR:-/tmp/swt-agent-pid-lock-$(id -u)-${LOCK_KEY}}"
...
cmd_register() {
  ...
  # Create .swt-planning if missing
