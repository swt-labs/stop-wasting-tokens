...
TODO_HARDCODED_HELPER_NEEDLE='/tmp/.swt-install-root-link-${SWT_SESSION_ID:-default}/scripts/todo-details.sh'
TODO_HARDCODED_PLANNING_GIT_NEEDLE='/tmp/.swt-install-root-link-${SWT_SESSION_ID:-default}/scripts/planning-git.sh'
TODO_PLANNING_GIT_CMD='bash "${PLUGIN_ROOT}/scripts/planning-git.sh" commit-boundary "add todo item" .swt-planning/config.json'
...
check "TODO-03" "todo command avoids preflight plugin-root resolver shell block" test ! "$(grep -c 'SWT_CACHE_ROOT=' "$TODO_CMD")" -gt 0
...
check "TODO-11" "todo command forbids direct per-file fallback writes" \
  grep -Fq '.swt-planning/todo-details/HASH.json' "$TODO_CMD"
...
check "TODO-15" "todo command reuses the planning-git unavailable warning" \
  grep -Fq 'SWT: planning-git.sh unavailable; skipping planning git boundary commit' "$TODO_CMD"
...
check "LIST-01" "list-todos command avoids preflight plugin-root resolver shell block" test ! "$(grep -c 'SWT_CACHE_ROOT=' "$LIST_CMD")" -gt 0
...
check "LIST-06" "list-todos displays slash command usage hints" \
  grep -qi 'swt cook N\|swt fix N\|swt debug N' "$LIST_CMD"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/swt-init-todo.XXXXXX")"
...
LIST_TMP="$(mktemp -d "${TMPDIR:-/tmp}/swt-list-ref.XXXXXX")"
trap 'rm -rf "$TMP_DIR" "$LIST_TMP"' EXIT
mkdir -p "$LIST_TMP/.swt-planning"
cat > "$LIST_TMP/.swt-planning/STATE.md" << 'HEREDOC'
# State
## Todos
...
check "LIST-04" "list-todos.sh parses ref tag into JSON ref field" \
  bash -c 'cd "$1" && SWT_PLANNING_DIR=.swt-planning bash "$2/scripts/list-todos.sh" 2>/dev/null | jq -e ".items[0].ref == \"abc12345\"" >/dev/null' _ "$LIST_TMP" "$ROOT"

check "LIST-05" "list-todos.sh strips ref tag from display text" \
  bash -c 'cd "$1" && SWT_PLANNING_DIR=.swt-planning bash "$2/scripts/list-todos.sh" 2>/dev/null | jq -e "(.display | test(\"ref:abc12345\") | not)" >/dev/null' _ "$LIST_TMP" "$ROOT"
