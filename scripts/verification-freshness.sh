...
verification_is_stale() {
  ...
  if ! _dirty=$(git status --porcelain --untracked-files=normal -- . ':!.swt-planning' ':!CLAUDE.md' 2>/dev/null); then
  ...
  if [ -n "$_vac" ]; then
    if ! _cur_commit=$(git log -1 --format='%H' -- . ':!.swt-planning' ':!CLAUDE.md' 2>/dev/null); then
  ...
  if ! _cur_commit_ts=$(git log -1 --format='%ct' -- . ':!.swt-planning' ':!CLAUDE.md' 2>/dev/null); then
    VERIFICATION_FRESHNESS_REASON="git_log_failed"
    return 0
  fi
