#!/bin/bash
set -u
# agent-health.sh — Track SWT agent health and recover from failures
#
# Usage:
...
orphan_recovery() {
  ...
  local key="${3:-}"
  local team_name="${4:-}"
  local tasks_dir="${CLAUDE_DIR:-${SWT_CONFIG_DIR:-$HOME/.claude}}/tasks"
  local advisory=""
