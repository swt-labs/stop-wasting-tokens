#!/usr/bin/env bash
# scripts/collect-diagnostics.sh — Collect SWT diagnostic context for bug reporting
...
#   $2  Project working directory (where .swt-planning/ may exist)
#
...
collect() {
  ...
  local cache_root="${SWT_CONFIG_DIR:-${HOME:-/tmp}/.claude}/plugins/cache/swt-marketplace/vbw"

  echo "=== SWT Diagnostic Report ==="
  ...
  if [ -f "$PROJECT_DIR/.swt-planning/.hook-errors.log" ]; then
    tail -20 "$PROJECT_DIR/.swt-planning/.hook-errors.log" 2>/dev/null || echo "(read error)"
  else
  ...
  if [ -f "$PROJECT_DIR/.swt-planning/.session-log.jsonl" ]; then
    tail -10 "$PROJECT_DIR/.swt-planning/.session-log.jsonl" 2>/dev/null || echo "(read error)"
  else
  ...
  if [ -f "$PROJECT_DIR/.swt-planning/.events/event-log.jsonl" ]; then
    tail -20 "$PROJECT_DIR/.swt-planning/.events/event-log.jsonl" 2>/dev/null || echo "(read error)"
  else
  ...
  if [ -f "$PROJECT_DIR/.swt-planning/.metrics/run-metrics.jsonl" ]; then
    tail -10 "$PROJECT_DIR/.swt-planning/.metrics/run-metrics.jsonl" 2>/dev/null || echo "(read error)"
  else
  ...
  if [ -f "$PROJECT_DIR/.swt-planning/config.json" ]; then
    if command -v jq >/dev/null 2>&1; then
      jq 'def walk(f): . as $in | if type == "object" then reduce keys[] as $k ({}; . + {($k): ($in[$k] | walk(f))}) | f elif type == "array" then map(walk(f)) | f else f end; walk(if type == "object" then with_entries(if (.key | test("key|token|secret|password|api_key"; "i")) then .value = "[REDACTED]" else . end) else . end)' \
        "$PROJECT_DIR/.swt-planning/config.json" 2>/dev/null || cat "$PROJECT_DIR/.swt-planning/config.json" 2>/dev/null || echo "(read error)"
    else
      cat "$PROJECT_DIR/.swt-planning/config.json" 2>/dev/null || echo "(read error)"
    fi
  else
  ...
  # --- Project State ---
  echo "--- Project State ---"
  if [ -d "$PROJECT_DIR/.swt-planning" ]; then
    if [ -f "$PROJECT_DIR/.swt-planning/STATE.md" ]; then
      echo "STATE.md:"
      cat "$PROJECT_DIR/.swt-planning/STATE.md" 2>/dev/null || echo "(read error)"
    else
      echo "STATE.md: (not found)"
    fi
    echo ""
    echo "Phases:"
    if [ -d "$PROJECT_DIR/.swt-planning/phases" ]; then
      ls -1 "$PROJECT_DIR/.swt-planning/phases/" 2>/dev/null || echo "(empty)"
    else
      echo "(no phases directory)"
    fi
  else
    echo "(.swt-planning/ not found — project not initialized)"
  fi
  echo ""

  # --- Hook Debug Log ---
  echo "--- Hook Debug Log (last 10) ---"
  if [ -f "$PROJECT_DIR/.swt-planning/.hook-debug.log" ]; then
    tail -10 "$PROJECT_DIR/.swt-planning/.hook-debug.log" 2>/dev/null || echo "(read error)"
  else
  ...
  local claude_dir="${SWT_CONFIG_DIR:-${HOME:-/tmp}/.claude}"
  local session_id="${SWT_SESSION_ID:-}"
  local session_key="${session_id:-default}"
  local session_link="/tmp/.swt-install-root-link-${session_key}"
  ...
  hook_cache=$(ls -1 "$claude_dir"/plugins/cache/*/scripts/hook-wrapper.sh 2>/dev/null | (sort -V 2>/dev/null || sort -t. -k1,1n -k2,2n -k3,3n) | tail -1 || true)
  hook_cpr="${SWT_INSTALL_ROOT:+$SWT_INSTALL_ROOT/scripts/hook-wrapper.sh}"
  for f in /tmp/.swt-install-root-link-*/scripts/hook-wrapper.sh; do
  ...
  if [ -n "$PLUGIN_ROOT" ] && [ -f "$PLUGIN_ROOT/VERSION" ] && [ -f "$PLUGIN_ROOT/scripts/lib/swt-cache-key.sh" ]; then
    local _diag_ver _diag_uid _diag_config_cache _diag_update_cache
    # shellcheck source=lib/swt-cache-key.sh
    . "$PLUGIN_ROOT/scripts/lib/swt-cache-key.sh"
    ...
    _diag_update_cache="/tmp/swt-update-check-${_diag_uid}-$(vbw_hash_path "$_diag_scripts_dir")"
  ...
  # Welcome marker
  echo "welcome_marker: $([ -f "$claude_dir/.swt-welcomed" ] && echo "YES" || echo "NO")"

  # Cache entry summary
  local cache_dir="${claude_dir}/plugins/cache/swt-marketplace/vbw"
  if [ -d "$cache_dir" ]; then
    local entry_count
  ...
  local mp_cache_root="${claude_dir}/plugins/cache/swt-marketplace/vbw"
  ...
  if [ -f "$mp_installed" ]; then
    if command -v jq >/dev/null 2>&1; then
      ...
      vbw_reg=$(jq -r '
        .plugins // {}
        | if has("vbw@swt-marketplace") then
            "vbw@swt-marketplace"
          else
            empty
          end
  ...
  if [ -n "$PLUGIN_ROOT" ] && [ -d "$PLUGIN_ROOT/agents" ]; then
    local agent_count
    agent_count=$(find "$PLUGIN_ROOT/agents" -name 'swt-*.md' 2>/dev/null | wc -l | tr -d ' ')
