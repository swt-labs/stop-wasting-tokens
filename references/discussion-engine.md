...
- `swt cook` — auto-detects discussion need from state
- `swt cook --discuss [N]` — explicit flag, targets phase N
- `swt discuss [N]` — standalone command
...
- Whether they typed `swt discuss` vs `swt cook`
...
2. `discussion_mode` is `"assumptions"` in `.swt-planning/config.json` → use assumptions path
3. `discussion_mode` is `"auto"` AND `.swt-planning/codebase/META.md` exists → use assumptions path
4. `discussion_mode` is `"questions"` or `"auto"` without codebase map → proceed to Step 2 (standard questions path)

**Codebase map guard:** If the assumptions path is activated but `.swt-planning/codebase/META.md` does not exist, display: "Assumptions mode works best with codebase context. Run `swt map` first for evidence-backed assumptions, or falling back to questions mode." Fall back to Step 2 (standard questions path).

### A1: Codebase Analysis

Read codebase context files relevant to the target phase:

- `.swt-planning/codebase/ARCHITECTURE.md` — architecture patterns, data flow
- `.swt-planning/codebase/PATTERNS.md` — code conventions, design patterns
- `.swt-planning/codebase/CONCERNS.md` — known issues, tech debt
...
- **Milestone scope** → `.swt-planning/CONTEXT.md` — written by Scope mode (`swt cook --scope`). Captures decomposition decisions (why N phases, ordering rationale), requirement-to-phase mapping, project-level key decisions, and deferred ideas. Archived with the milestone.
- **Phase discussion** → `.swt-planning/phases/{NN}-{slug}/{NN}-CONTEXT.md` — written by this Discussion Engine. Captures gray-area decisions, user preferences, and scope boundaries for a single phase.
