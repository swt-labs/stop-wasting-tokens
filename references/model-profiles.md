...
**Purpose:** Control AI model selection for SWT agents to optimize cost vs quality tradeoff.

## Overview

SWT spawns 7 specialized agents (Lead, Dev, QA, Scout, Debugger, Architect, Docs) via the Task tool. Model profiles determine which Claude model each agent uses. Three preset profiles cover common use cases, with per-agent overrides for advanced customization.
...
**Syntax (via swt config):**

```
swt config model_override <agent> <model>
```

**Example:**

```
swt config model_override dev opus
```
...
**Clearing overrides:**
Switch to a different profile and back, or manually edit .swt-planning/config.json.
...
```
swt config model_profile <quality|balanced|budget>
```

Displays before/after cost impact estimate.

**Config file location:**
`.swt-planning/config.json` -- fields: `model_profile` (string), `model_overrides` (object)
