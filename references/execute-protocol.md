# SWT Execution Protocol

Loaded on demand by swt cook Execute mode. Not a user-facing command.

## Runtime Plugin Root Resolution (required once per Execute run)

Resolve and validate `SWT_INSTALL_ROOT` once before running script commands below:
...
if [ -n "${SWT_INSTALL_ROOT:-}" ] && [ -f "${SWT_INSTALL_ROOT}/scripts/hook-wrapper.sh" ]; then
  SWT_INSTALL_ROOT="${SWT_INSTALL_ROOT}"
fi
if [ -z "$SWT_INSTALL_ROOT" ] && [ -f "${SWT_CACHE_ROOT}/local/scripts/hook-wrapper.sh" ]; then
  SWT_INSTALL_ROOT="${SWT_CACHE_ROOT}/local"
fi
if [ -z "$SWT_INSTALL_ROOT" ]; then
  VERSION_DIR=$(find "${SWT_CACHE_ROOT}" -maxdepth 1 -mindepth 1 2>/dev/null | awk -F/ '{print $NF}' | grep -E '^[0-9]+(\.[0-9]+)*$' | sort -t. -k1,1n -k2,2n -k3,3n | tail -1)
  if [ -n "$VERSION_DIR" ] && [ -f "${SWT_CACHE_ROOT}/${VERSION_DIR}/scripts/hook-wrapper.sh" ]; then
    SWT_INSTALL_ROOT="${SWT_CACHE_ROOT}/${VERSION_DIR}"
  else
    FALLBACK_DIR=$(find "${SWT_CACHE_ROOT}" -maxdepth 1 -mindepth 1 2>/dev/null | awk -F/ '{print $NF}' | sort | tail -1)
    [ -n "$FALLBACK_DIR" ] && [ -f "${SWT_CACHE_ROOT}/${FALLBACK_DIR}/scripts/hook-wrapper.sh" ] && SWT_INSTALL_ROOT="${SWT_CACHE_ROOT}/${FALLBACK_DIR}"
  fi
fi
SESSION_LINK="/tmp/.swt-install-root-link-${SWT_SESSION_ID:-default}"
if [ -z "$SWT_INSTALL_ROOT" ] && [ -f "${SESSION_LINK}/scripts/hook-wrapper.sh" ]; then
...
if [ -z "$SWT_INSTALL_ROOT" ]; then
  ...
  [ -n "$PLUGIN_DIR_PATH" ] && [ -f "$PLUGIN_DIR_PATH/scripts/hook-wrapper.sh" ] && SWT_INSTALL_ROOT="$PLUGIN_DIR_PATH"
fi

if [ -z "$SWT_INSTALL_ROOT" ] || [ ! -d "$SWT_INSTALL_ROOT" ]; then
  echo "SWT: plugin root resolution failed (checked SWT_INSTALL_ROOT, cache local/, versioned dirs, symlink fallback, process tree)." >&2
  exit 1
fi

# Canonicalize to real path — survives cache symlink deletion mid-session
SWT_INSTALL_ROOT=$(cd "$SWT_INSTALL_ROOT" 2>/dev/null && pwd -P || echo "$SWT_INSTALL_ROOT")
```

All runtime script invocations below assume `SWT_INSTALL_ROOT` is set.

### Step 2: Load plans and detect resume state

**Orchestrator read-scope boundary:** You may ONLY read planning/state artifacts: `*-PLAN.md`, `*-SUMMARY.md`, `*-RESEARCH.md`, `STATE.md`, `ROADMAP.md`, `REQUIREMENTS.md`, `.execution-state.json`, `.context-*.md`, `config.json`, and `.swt-planning/` metadata. Do NOT read product source files (application code, tests, configs outside `.swt-planning/`). If you need to understand product code to make a routing or sequencing decision, that understanding must come from Dev — delegate it via a task.
...
- If `RISK=low` AND `TASK_COUNT<=3` AND effort is not `thorough`: write route `turbo` for that plan in `$ROUTE_MAP` and log numeric metrics:
  `bash "${SWT_INSTALL_ROOT}/scripts/collect-metrics.sh" smart_route {phase} {plan} risk=$RISK tasks=$TASK_COUNT routed=turbo 2>/dev/null || true`
- Otherwise: omit the route-map entry or write route `delegate`; log non-turbo delegated metrics as:
  `bash "${SWT_INSTALL_ROOT}/scripts/collect-metrics.sh" smart_route {phase} {plan} risk=$RISK tasks=$TASK_COUNT routed=team 2>/dev/null || true`
...
ROUTING=$(bash "${SWT_INSTALL_ROOT}/scripts/resolve-execute-delegation-mode.sh" \
  --phase-dir "{phase_dir}" \
  --config .swt-planning/config.json \
  --execution-state .swt-planning/.execution-state.json \
  "${ROUTE_ARGS[@]}" \
  --segments)
```
...
1. **True team mode**
   ...
   - **Pre-TeamCreate cleanup** (remove orphaned SWT team directories from prior sessions before creating a new team):
     ```bash
     bash "${SWT_INSTALL_ROOT}/scripts/clean-stale-teams.sh" 2>/dev/null || true
     ```
   - Set `TEAM_NAME` from the segment (`team_name`) or default to `"swt-phase-{NN}"`.
   ...
   - Persist the actual runtime mode:
     ```bash
     bash "${SWT_INSTALL_ROOT}/scripts/delegated-workflow.sh" set execute {segment_effort} team "$TEAM_NAME"
     ```
...
2. **Explicit non-team mode**
   ...
   - For serialized delegate segments (`route=delegate`, `delegation_mode=subagent`), persist the actual runtime mode as subagent, skip TeamCreate, spawn one Dev subagent, and wait for completion before the next spawn:
     ```bash
     bash "${SWT_INSTALL_ROOT}/scripts/delegated-workflow.sh" set execute {segment_effort} subagent
     ```
   - For `turbo` or internal `direct` segments (`delegation_mode=direct`), update `.execution-state.json.effort` to `turbo` or `direct` before orchestrator product-file writes; keep `phase_effort` unchanged, persist the actual runtime mode as direct, and restore `.execution-state.json.effort` to `phase_effort` before the next non-direct segment:
     ```bash
     bash "${SWT_INSTALL_ROOT}/scripts/delegated-workflow.sh" set execute {segment_effort} direct
     ```
...
- If non-empty array (not `[]`): pass package paths to context compilation for scoped file inclusion.
  Log: `bash "${SWT_INSTALL_ROOT}/scripts/collect-metrics.sh" monorepo_route {phase} packages=$PACKAGES 2>/dev/null || true`
...
- **Before each task:** Run the `pre-task` action:
  ```bash
  CP_RESULT=$(bash "${SWT_INSTALL_ROOT}/scripts/control-plane.sh" pre-task {phase} {plan} {task} \
...
- **After each task:** Run the `post-task` action:
  ```bash
  CP_RESULT=$(bash "${SWT_INSTALL_ROOT}/scripts/control-plane.sh" post-task {phase} {plan} {task} \
...
- After context compilation, enforce per-role token budgets. Pass the contract path and task number for per-task budget computation:
  ...
  ```
  Where `{contract_path}` is `.swt-planning/.contracts/{phase}-{plan}.json` (generated by generate-contract.sh in Step 3) and `{task_number}` is the current task being executed (1-based). When no contract is available, omit the contract_path and task_number arguments (per-role fallback).
- Same for QA context: `bash "${SWT_INSTALL_ROOT}/scripts/token-budget.sh" qa {phase-dir}/.context-qa.md {contract_path} {task_number} > ...`
...
- **Escalation:** When overage occurs, token-budget.sh emits a `token_cap_escalated` event and reduces the remaining budget for subsequent tasks in the plan. The budget reduction state is stored in `.swt-planning/.token-state/{phase}-{plan}.json`. Escalation is advisory only -- execution continues regardless.
- **Cleanup:** At phase end, clean up token state: `rm -f .swt-planning/.token-state/*.json 2>/dev/null || true`
...
DEV_MAX_TURNS=$(bash "${SWT_INSTALL_ROOT}/scripts/resolve-agent-max-turns.sh" dev .swt-planning/config.json "{effort}")
if [ $? -ne 0 ]; then echo "$DEV_MAX_TURNS" >&2; exit 1; fi

QA_MODEL=$(bash "${SWT_INSTALL_ROOT}/scripts/resolve-agent-model.sh" qa .swt-planning/config.json "${SWT_INSTALL_ROOT}/config/model-profiles.json")
if [ $? -ne 0 ]; then echo "$QA_MODEL" >&2; exit 1; fi
QA_MAX_TURNS=$(bash "${SWT_INSTALL_ROOT}/scripts/resolve-agent-max-turns.sh" qa .swt-planning/config.json "{effort}")
...
description: |
  ...
  <skill_follow_up_files>{If one or more skills were preselected, run `bash "${SWT_INSTALL_ROOT}/scripts/extract-skill-follow-up-files.sh" "{all preselected skill names from the activation block}" 2>/dev/null || true` before spawning and replace this block with the emitted absolute follow-up file paths. Omit this block when the helper prints nothing.}</skill_follow_up_files>
...
**CRITICAL:** Set `subagent_type: "swt:swt-dev"` and `model: "${DEV_MODEL}"` on the live spawn call when spawning Dev teammates. If `DEV_MAX_TURNS` is non-empty, also pass `maxTurns: ${DEV_MAX_TURNS}`. If `DEV_MAX_TURNS` is empty, do NOT include maxTurns (omitting it = unlimited).
**CRITICAL:** When true team mode is active, pass `team_name: "swt-phase-{NN}"` and `name: "dev-{MM}"` on the live spawn call. If the live spawn tool is `Agent`, those parameters belong on `Agent(...)`. If the live spawn tool is `TaskCreate`, put the same parameters there. Team mode without `team_name` is invalid.
...
- **Per plan:** Assess risk and resolve gate policy:
  ```bash
  RISK=$(bash "${SWT_INSTALL_ROOT}/scripts/assess-plan-risk.sh" {plan_path} 2>/dev/null || echo "medium")
  GATE_POLICY=$(bash "${SWT_INSTALL_ROOT}/scripts/resolve-gate-policy.sh" {effort} $RISK {autonomy} 2>/dev/null || echo '{}')
  ```
...
- Log to metrics: `bash "${SWT_INSTALL_ROOT}/scripts/collect-metrics.sh" gate_policy {phase} {plan} risk=$RISK qa_tier=$QA_TIER approval=$APPROVAL 2>/dev/null || true`
...
- At plan complete: `bash "${SWT_INSTALL_ROOT}/scripts/log-event.sh" plan_end {phase} {plan} status=complete 2>/dev/null || true`
- At plan failure: `bash "${SWT_INSTALL_ROOT}/scripts/log-event.sh" plan_end {phase} {plan} status=failed 2>/dev/null || true`
- On error: `bash "${SWT_INSTALL_ROOT}/scripts/log-event.sh" error {phase} {plan} message={error_summary} 2>/dev/null || true`
...
- `artifact_written`: after writing/modifying a file: `log-event.sh artifact_written {phase} {plan} path={file} task_id={id}`
  - Also register in artifact registry: `bash "${SWT_INSTALL_ROOT}/scripts/artifact-registry.sh" register {file} {event_id} {phase} {plan}`
...
- **Before each task:** Validate task start:
  `bash "${SWT_INSTALL_ROOT}/scripts/validate-contract.sh" start {contract_path} {task_number} 2>/dev/null || true`
- **After each task:** Validate modified files against contract:
  `bash "${SWT_INSTALL_ROOT}/scripts/validate-contract.sh" end {contract_path} {task_number} {modified_files...} 2>/dev/null || true`
...
- **Pre-task gate sequence (before each task starts):**
  ...
  - If any gate fails (exit 2): attempt auto-repair:
    `REPAIR=$(bash "${SWT_INSTALL_ROOT}/scripts/auto-repair.sh" {gate_type} {phase} {plan} {task} {contract_path})`
...
- **During long-running tasks** (>2 minutes estimated): renew lease periodically:
  `bash "${SWT_INSTALL_ROOT}/scripts/lease-lock.sh" renew {task_id} 2>/dev/null || true`
- Check for expired leases before acquiring: `bash "${SWT_INSTALL_ROOT}/scripts/lease-lock.sh" check {task_id} {claimed_files...} 2>/dev/null || true`
...
- Artifact registration: after each file write during task execution, register the artifact:
  ```bash
  bash "${SWT_INSTALL_ROOT}/scripts/artifact-registry.sh" register {file_path} {event_id} {phase} {plan}
  ```
...
for summary_file in {phase-dir}/*-SUMMARY.md; do
  ...
  devs=""
  if [ -f "${SWT_INSTALL_ROOT}/scripts/summary-utils.sh" ]; then
    # shellcheck source=/dev/null
    . "${SWT_INSTALL_ROOT}/scripts/summary-utils.sh"
  fi
...
This writes `{phase-dir}/known-issues.json`. The human-readable `Discovered Issues` block later in the execute summary is supplemental — the JSON registry is the authoritative phase backlog. Unresolved issues that survive QA and remediation are auto-promoted to `STATE.md ## Todos` via `promote-todos`, making them visible in `swt list-todos` and `swt resume`.
...
`bash "${SWT_INSTALL_ROOT}/scripts/compile-context.sh" {phase} qa {phases_dir}`
...
**CRITICAL:** Set `subagent_type: "swt:swt-qa"` and `model: "${QA_MODEL}"` in the Task tool invocation when spawning QA agents. If `QA_MAX_TURNS` is non-empty, also pass `maxTurns: ${QA_MAX_TURNS}`. If `QA_MAX_TURNS` is empty, do NOT include maxTurns (omitting it = unlimited).
**CRITICAL:** When true team mode is active, pass `team_name: "swt-phase-{NN}"` and `name: "qa"` (or `name: "qa-wave{W}"` for per-wave QA) parameters to each QA Task tool invocation.
...
4. Log event: `bash "${SWT_INSTALL_ROOT}/scripts/log-event.sh" shutdown_received {phase} team={team_name} approved={count} rejected={count} 2>/dev/null || true`
...
> **Runtime enforcement limitation:** Claude Code does not expose agent-team message tool calls (e.g., `SendMessage`) to `PreToolUse`/`PostToolUse` hooks with stable `tool_name` values. Therefore SWT cannot hook-validate malformed shutdown responses at runtime. Enforcement relies on: (1) mechanical SendMessage instructions in all 6 agent prompts, (2) compaction-instructions.sh reminders that survive context compaction, (3) orchestrator retry (re-send if teammate responds in plain text), and (4) `swt doctor --cleanup` as a recovery path for stuck teams.
...
if [ -f "$VERIFY_SCRIPT" ]; then
  _vsc_out="$(bash "$VERIFY_SCRIPT" .swt-planning --mode advisory 2>/dev/null || true)"
  if [ -n "$_vsc_out" ] && echo "$_vsc_out" | jq -e '.verdict == "fail"' >/dev/null 2>&1; then
    echo "SWT state-consistency warning: $(echo "$_vsc_out" | jq -c '.failed_checks')" >&2
  fi
fi
```
...
if [ -f "$PG_SCRIPT" ]; then
  bash "$PG_SCRIPT" commit-boundary "complete phase {NN}" .swt-planning/config.json
else
...
PG_SCRIPT="${SWT_INSTALL_ROOT}/scripts/planning-git.sh"
if [ -f "$PG_SCRIPT" ]; then
  bash "$PG_SCRIPT" push-after-phase .swt-planning/config.json
else
...
Follow @${SWT_INSTALL_ROOT}/references/swt-brand-essentials.md — Phase Banner (double-line box), ◆ running, ✓ complete, ✗ failed, ○ skipped, Metrics Block, Next Up Block, no ANSI color codes.
