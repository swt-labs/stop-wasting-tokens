# SWT Verification Protocol

Authoritative spec for SWT's verification pipeline. QA agent persists results to `VERIFICATION.md` by calling `write-verification.sh` directly (the orchestrator does not persist).

## 1. Contexts

- **Post-build:** Auto after `swt cook` execute mode (unless `--skip-qa` or turbo)
- **On-demand:** Triggered by `swt cook`, `swt debug`, or the hidden internal `swt qa` protocol command
