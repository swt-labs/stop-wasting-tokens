...
description: Set up environment, scaffold .swt-planning, detect project context, and bootstrap project-defining files.
argument-hint:
...
Existing state:
```
!`ls -la .swt-planning 2>/dev/null || echo "No .swt-planning directory"`
```
Project files:
```
...
Skills:
```
!`if [ -n "${SWT_CONFIG_DIR:-}" ]; then _cd="$SWT_CONFIG_DIR"; elif [ -d "$HOME/.config/claude-code" ]; then _cd="$HOME/.config/claude-code"; else _cd="$HOME/.claude"; fi; ls "$_cd/skills/" 2>/dev/null || echo "No global skills"`
```
```
...
1. **Already initialized:** If .swt-planning/config.json exists, STOP: "SWT is already initialized. Use swt config to modify settings or swt cook to start building."
2. **jq required:** `command -v jq` via Bash. If missing, STOP: "SWT requires jq. Install: macOS `brew install jq`, Linux `apt install jq`, Manual: https://jqlang.github.io/jq/download/ — then re-run swt init." Do NOT proceed without jq.
3. **Brownfield detection:** Check for existing source files (stop at first match):
   ...
   - No git: Glob `**/*.*` excluding `.swt-planning/`, `.claude/`, `node_modules/`, `.git/` — any match = BROWNFIELD=true
...
**Resolve config directory:** Try in order: env var `SWT_CONFIG_DIR` (if set, even if directory does not yet exist), `~/.config/claude-code` (if exists), otherwise `~/.claude`. Store result as `CLAUDE_DIR`. Use it for all config paths in this command.
...
- Not enabled: AskUserQuestion: "⚠ Agent Teams is not enabled\n\nSWT uses Agent Teams for parallel builds and codebase mapping.\nEnable it now?"
...
|-------|-----------|--------|
| HAS_SWT | Value contains `swt-statusline` | Display "✓ Statusline — installed", skip to 0c |
| HAS_OTHER | Non-empty, no `swt-statusline` | AskUserQuestion (mention replacement) |
| EMPTY | Missing/null/empty | AskUserQuestion |

AskUserQuestion text: "○ SWT includes a custom status line showing phase progress, context usage, cost, duration, and more — updated after every response. Install it?" (If HAS_OTHER, mention existing statusline would be replaced.)
...
If declined: display "○ Skipped. Run swt config to install it later."
...
- .swt-planning/gsd-archive/ is created before scaffold creates .swt-planning/
...
- **Found:** proceed with import flow:
  ...
  2. AskUserQuestion: "GSD project detected. Import work history?\n\nThis will copy .planning/ to .swt-planning/gsd-archive/ for reference.\nYour original .planning/ directory will remain untouched."
     - Options: "Import (Recommended)" / "Skip"
  3. If user declines:
     - Display: "○ GSD import skipped"
     - Proceed to Step 1
  4. If user approves:
     - Create directory: `mkdir -p .swt-planning/gsd-archive`
     - Copy contents: `cp -r .planning/* .swt-planning/gsd-archive/`
     - Display: "◆ Generating index..."
      - Run: `bash "{plugin-root}/scripts/generate-gsd-index.sh"`
     - Display: "✓ GSD project archived to .swt-planning/gsd-archive/ (indexed)"
     - Set GSD_IMPORTED=true flag for later steps
     - Proceed to Step 1

### Step 1: Scaffold directory

Read each template from `{plugin-root}/templates/` and write to .swt-planning/:

| Target | Source |
|--------|--------|
| .swt-planning/PROJECT.md | `{plugin-root}/templates/PROJECT.md` |
| .swt-planning/REQUIREMENTS.md | `{plugin-root}/templates/REQUIREMENTS.md` |
| .swt-planning/ROADMAP.md | `{plugin-root}/templates/ROADMAP.md` |
| .swt-planning/STATE.md | `{plugin-root}/templates/STATE.md` |
| .swt-planning/config.json | `{plugin-root}/config/defaults.json` |

Create `.swt-planning/phases/`. Ensure config.json includes `"prefer_teams": "auto"` and `"model_profile": "quality"`.

AskUserQuestion (single select):
- "How should SWT planning artifacts be tracked in git?"
  - `manual` (default): don't auto-ignore the whole `.swt-planning/` directory or auto-commit planning files
  - `ignore`: keep the whole `.swt-planning/` directory ignored in root `.gitignore`
  - `commit`: track `.swt-planning/` + `CLAUDE.md` at helper-backed planning boundaries (bootstrap, planning checkpoints, todo adds, archive)
...
Write selected values to `.swt-planning/config.json`:

```bash
jq '.planning_tracking = "'"$PLANNING_TRACKING"'" | .auto_push = "'"$AUTO_PUSH"'"' .swt-planning/config.json > .swt-planning/config.json.tmp && mv .swt-planning/config.json.tmp .swt-planning/config.json
```
...
**1.7a. Detection:** `[ -d "${SWT_CONFIG_DIR:-$HOME/.claude}/commands/gsd" ] || [ -d "$HOME/.config/claude-code/commands/gsd" ] || [ -d ".planning" ] || [ -d ".swt-planning/gsd-archive" ]`
- None true: GSD_DETECTED=false, display nothing, skip to Step 2
- Any true: GSD_DETECTED=true, proceed to 1.7b

**1.7b. Consent:** AskUserQuestion: "GSD detected. Enable plugin isolation?\n\nThis adds a PreToolUse hook that prevents GSD commands and agents from\nreading or writing files in .swt-planning/. SWT commands are unaffected."
Options: "Enable (Recommended)" / "Skip". If declined: "○ GSD isolation skipped", skip to Step 2.

**1.7c. Create isolation:** If approved:
1. `echo "enabled" > .swt-planning/.gsd-isolation`
2. `echo "session" > .swt-planning/.swt-session`
3. Display: `✓ GSD isolation enabled` + `✓ .swt-planning/.gsd-isolation (flag)` + `✓ Plugin Isolation section will be added to CLAUDE.md in Step 3.5`
...
- Count source files by extension (Glob), excluding .swt-planning/, node_modules/, .git/, vendor/, dist/, build/, target/, .next/, __pycache__/, .venv/, coverage/
...
**3b.** If `.swt-planning/codebase/STACK.md` exists, read it and merge additional stack components into detected_stack[].

**3b2. Auto-detect conventions:** If `.swt-planning/codebase/PATTERNS.md` exists:
- Read PATTERNS.md, ARCHITECTURE.md, STACK.md, CONCERNS.md
- Extract conventions per `{plugin-root}/commands/teach.md` (Step R2)
- Write `.swt-planning/conventions.json`. Display: `✓ {count} conventions auto-detected from codebase`

If greenfield: write `{"conventions": []}`. Display: `○ Conventions — none yet (add with swt teach)`
...
**3d. Unified skill prompt:** Combine curated (from 2b) + registry (from 3c) results into single AskUserQuestion multiSelect. Tag `(curated)` or `(registry)`. Use max 4 visible choices total, including `Skip`; if more than 3 skills are candidates, show the top 3 plus `Skip` and point broader discovery to `swt skills`. Install selected: `npx skills add <skill> -g -y`.

### Step 3.5: Generate bootstrap CLAUDE.md

SWT needs its rules and state sections in a CLAUDE.md file. swt cook regenerates later with project content.

**Brownfield handling:** Read root `CLAUDE.md` via the Read tool.
- **Exists:** The user already has a CLAUDE.md. Do NOT overwrite it and do NOT assume the first heading/core-value lines belong to SWT. Preserve all user-authored content verbatim. Only refresh exact canonical SWT-owned sections already emitted by SWT (`## Active Context`, `## SWT Rules`, `## Plugin Isolation`) and add `## Code Intelligence` only if no Code Intelligence heading/guidance already exists anywhere in the file. Display `✓ CLAUDE.md (SWT sections refreshed in place)`.
...
- If GSD_IMPORTED=true: Display "✓ GSD project archived ({file count} files, indexed)" where file count = `find .swt-planning/gsd-archive -type f | wc -l`, then display sub-bullet: "  • Index: .swt-planning/gsd-archive/INDEX.json"
...
2. **GSD_MIGRATION:** `.swt-planning/gsd-archive/` directory exists (created in Step 0.5). Has GSD work history to import.
3. **BROWNFIELD:** BROWNFIELD=true AND `.swt-planning/codebase/` directory exists (created in Step 2c mapping). Has codebase context to infer from.
4. **HYBRID:** BROWNFIELD=true but `.swt-planning/codebase/` does not exist. Edge case — should not occur after Step 2c, but handle gracefully by treating as GREENFIELD.

Check conditions in order (GSD_MIGRATION first since a GSD project may also be brownfield):

```
if [ -d .swt-planning/gsd-archive ]; then SCENARIO=GSD_MIGRATION
elif [ "$BROWNFIELD" = "true" ] && [ -d .swt-planning/codebase ]; then SCENARIO=BROWNFIELD
...
- Run inference: `bash "{plugin-root}/scripts/infer-project-context.sh" .swt-planning/codebase/ "$(pwd)"`
- Capture JSON output to `.swt-planning/inference.json` via Bash
...
- Run GSD inference: `bash "{plugin-root}/scripts/infer-gsd-summary.sh" .swt-planning/gsd-archive/`
- Capture JSON output to `.swt-planning/gsd-inference.json` via Bash
- If `.swt-planning/codebase/` exists, also run: `bash "{plugin-root}/scripts/infer-project-context.sh" .swt-planning/codebase/ "$(pwd)"`
  - Capture to `.swt-planning/inference.json`
...
Write the final confirmed/corrected data to `.swt-planning/inference.json` for Step 7 consumption.
...
- Read `.swt-planning/inference.json` to get confirmed project context
- Extract: NAME from `name.value`, DESCRIPTION from `purpose.value`
- If GSD_MIGRATION: read `.swt-planning/gsd-inference.json` for milestone/phase context
...
- Run: `bash "{plugin-root}/scripts/bootstrap/bootstrap-project.sh" .swt-planning/PROJECT.md "$NAME" "$DESCRIPTION"`
- Display: `✓ PROJECT.md`

**7c. Generate REQUIREMENTS.md:**
- Create `.swt-planning/discovery.json` with format: `{"answered": [...], "inferred": [...]}`
  - `answered`: array of requirement strings from user answers
  - `inferred`: array of `{"text": "...", "priority": "Must-have"}` from inference features
- Run: `bash "{plugin-root}/scripts/bootstrap/bootstrap-requirements.sh" .swt-planning/REQUIREMENTS.md .swt-planning/discovery.json`
- Display: `✓ REQUIREMENTS.md`

**7d. Generate ROADMAP.md:**
- Create `.swt-planning/phases.json` with format: `[{"name": "...", "goal": "...", "requirements": [...], "success_criteria": [...]}]`
...
- Run: `bash "{plugin-root}/scripts/bootstrap/bootstrap-roadmap.sh" .swt-planning/ROADMAP.md "$NAME" .swt-planning/phases.json`
- Display: `✓ ROADMAP.md`
...
- Run: `bash "{plugin-root}/scripts/bootstrap/bootstrap-state.sh" .swt-planning/STATE.md "$NAME" "$MILESTONE_NAME" "$PHASE_COUNT"`
- Display: `✓ STATE.md`

**7f. Generate/update CLAUDE.md:**
- Extract `CORE_VALUE` from `.swt-planning/PROJECT.md` (`grep -m1 '^\*\*Core value:\*\*' .swt-planning/PROJECT.md | sed 's/^\*\*Core value:\*\* *//'`)
- If root CLAUDE.md exists: pass it as EXISTING_PATH to preserve non-SWT content
...
- Remove `.swt-planning/discovery.json`, `.swt-planning/phases.json`, `.swt-planning/inference.json`, `.swt-planning/gsd-inference.json` (if they exist)
...
<!-- Final summary replaces old Step 4 auto-launch of swt cook -->
<!-- User now has full project-defining files and can run swt cook when ready -->

Display a banner per @${SWT_INSTALL_ROOT}/references/swt-brand-essentials.md with the title "SWT Initialization Complete".

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
SWT Initialization Complete
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
**Next steps:**
```
➜ Next: Run swt cook to start planning your first milestone
  Or:   Run swt status to review project state
```

## Output Format

Follow @${SWT_INSTALL_ROOT}/references/swt-brand-essentials.md — Phase Banner (double-line box), File Checklist (✓), ○ for pending, Next Up Block, no ANSI color codes.
