...
description: View and modify SWT configuration including effort profile, verification tier, and skill-hook wiring.
argument-hint: [setting value]
allowed-tools: Read, Write, Edit, Bash, Glob, AskUserQuestion
---

# SWT Config $ARGUMENTS
...
```
!`cat .swt-planning/config.json 2>/dev/null || echo "No config found -- run swt init first"`
```
...
```bash
MIGRATED_COUNT=$(bash "{plugin-root}/scripts/migrate-config.sh" --print-added .swt-planning/config.json 2>/dev/null)
if [ $? -ne 0 ] || [ -z "${MIGRATED_COUNT:-}" ]; then
  echo "⚠ Config migration failed (invalid JSON). Fix .swt-planning/config.json, then retry swt config"
  exit 0
fi

if [ "$MIGRATED_COUNT" -gt 0 ]; then
  echo "✓ Added $MIGRATED_COUNT new settings from defaults (run swt config to see them)"
fi
```
...
```bash
PROFILE=$(jq -r '.model_profile // "quality"' .swt-planning/config.json)
echo ""
echo "Model Profile: $PROFILE"
echo "Agent Models:"
# Resolve each agent model
LEAD=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" lead .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
DEV=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" dev .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
QA=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" qa .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
SCOUT=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" scout .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
DEBUGGER=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" debugger .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
ARCHITECT=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" architect .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
...
if [ "$(jq -r '.model_overrides.lead // ""' .swt-planning/config.json)" != "" ]; then LEAD_DISPLAY="${LEAD}*"; fi
if [ "$(jq -r '.model_overrides.dev // ""' .swt-planning/config.json)" != "" ]; then DEV_DISPLAY="${DEV}*"; fi
if [ "$(jq -r '.model_overrides.qa // ""' .swt-planning/config.json)" != "" ]; then QA_DISPLAY="${QA}*"; fi
if [ "$(jq -r '.model_overrides.scout // ""' .swt-planning/config.json)" != "" ]; then SCOUT_DISPLAY="${SCOUT}*"; fi
if [ "$(jq -r '.model_overrides.debugger // ""' .swt-planning/config.json)" != "" ]; then DEBUGGER_DISPLAY="${DEBUGGER}*"; fi
if [ "$(jq -r '.model_overrides.architect // ""' .swt-planning/config.json)" != "" ]; then ARCHITECT_DISPLAY="${ARCHITECT}*"; fi
...
After a core setting value is chosen, continue to Step 3 and apply it there with the same validation, write behavior, and side effects as the `swt config <setting> <value>` path below. Step 4 remains the no-args tail behavior after that shared apply step.
...
```bash
CURRENT_PROFILE=$(jq -r '.model_profile // "quality"' .swt-planning/config.json)
PROFILES_PATH="{plugin-root}/config/model-profiles.json"

# Get current models (before changes)
LEAD_OLD=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" lead .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
DEV_OLD=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" dev .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
QA_OLD=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" qa .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
SCOUT_OLD=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" scout .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
DEBUGGER_OLD=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" debugger .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
ARCHITECT_OLD=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" architect .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
...
```bash
CURRENT_LEAD=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" lead .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
CURRENT_DEV=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" dev .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
CURRENT_QA=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" qa .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
CURRENT_SCOUT=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" scout .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
```
...
```bash
CURRENT_DEBUGGER=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" debugger .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
CURRENT_ARCHITECT=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" architect .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
```
...
```bash
if ! jq -e '.model_overrides' .swt-planning/config.json >/dev/null 2>&1; then
  jq '.model_overrides = {}' .swt-planning/config.json > .swt-planning/config.json.tmp && mv .swt-planning/config.json.tmp .swt-planning/config.json
fi
```
...
```bash
jq ".model_overrides.lead = \"$LEAD_MODEL\"" .swt-planning/config.json > .swt-planning/config.json.tmp && mv .swt-planning/config.json.tmp .swt-planning/config.json
echo "✓ Model override: lead ➜ $LEAD_MODEL"

jq ".model_overrides.dev = \"$DEV_MODEL\"" .swt-planning/config.json > .swt-planning/config.json.tmp && mv .swt-planning/config.json.tmp .swt-planning/config.json
echo "✓ Model override: dev ➜ $DEV_MODEL"

jq ".model_overrides.qa = \"$QA_MODEL\"" .swt-planning/config.json > .swt-planning/config.json.tmp && mv .swt-planning/config.json.tmp .swt-planning/config.json
echo "✓ Model override: qa ➜ $QA_MODEL"

jq ".model_overrides.scout = \"$SCOUT_MODEL\"" .swt-planning/config.json > .swt-planning/config.json.tmp && mv .swt-planning/config.json.tmp .swt-planning/config.json
echo "✓ Model override: scout ➜ $SCOUT_MODEL"

jq ".model_overrides.debugger = \"$DEBUGGER_MODEL\"" .swt-planning/config.json > .swt-planning/config.json.tmp && mv .swt-planning/config.json.tmp .swt-planning/config.json
echo "✓ Model override: debugger ➜ $DEBUGGER_MODEL"

jq ".model_overrides.architect = \"$ARCHITECT_MODEL\"" .swt-planning/config.json > .swt-planning/config.json.tmp && mv .swt-planning/config.json.tmp .swt-planning/config.json
...
- If mismatch: AskUserQuestion "Settings no longer match '{profile}'. Save as new profile?" → "Save" (route to swt profile save) or "No" (set active_profile to "custom")
...
jq ".max_uat_remediation_rounds = ${CANONICAL_VALUE}" .swt-planning/config.json > .swt-planning/config.json.tmp && mv .swt-planning/config.json.tmp .swt-planning/config.json
...
# Get current profile
OLD_PROFILE=$(jq -r '.model_profile // "quality"' .swt-planning/config.json)
...
# Update config.json
jq ".model_profile = \"$PROFILE\"" .swt-planning/config.json > .swt-planning/config.json.tmp && mv .swt-planning/config.json.tmp .swt-planning/config.json
...
OLD_MODEL=$(bash "{plugin-root}/scripts/resolve-agent-model.sh" "$AGENT" .swt-planning/config.json "{plugin-root}/config/model-profiles.json")
...
jq ".model_overrides.$AGENT = \"$MODEL\"" .swt-planning/config.json > .swt-planning/config.json.tmp && mv .swt-planning/config.json.tmp .swt-planning/config.json
...
Four flags control what the SWT statusline shows:
...
Injected default is `false`, and runtime fallback is also unlimited when the persisted value is absent or malformed. `swt config` rejects malformed interactive input instead of writing it.
...
Follow @${SWT_INSTALL_ROOT}/references/swt-brand-essentials.md — single-line box, ✓ success, ⚠ invalid, ➜ transitions, no ANSI.
