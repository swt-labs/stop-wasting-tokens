...
2. **Restricted mode:** If the current permission mode does not allow Bash execution, STOP: "`swt list-todos` needs Bash access to run helper scripts. If you're in read-only or another restricted mode, switch to a write-enabled mode (for example bypass permissions) and rerun the command."
