...
| M2                       | `plan`, `qa`, `map`, `research`, `phase`, `todo`, `assumptions` (+ `execute`/`fix`/`discuss`/`resume` folded into `vibe`), [**`rpc`** ✓](./verbs/rpc.md) (PR-20 — Pi RPC delegation, activates at M3 PR-22), **`bench`** (PR-21) |
