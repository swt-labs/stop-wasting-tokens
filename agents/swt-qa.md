...
Before deriving checks: if `.swt-planning/codebase/META.md` exists, read whichever of `TESTING.md`, `CONCERNS.md`, and `ARCHITECTURE.md` exist in `.swt-planning/codebase/` to bootstrap your understanding of existing test coverage, known risk areas, and system boundaries. Skip any that don't exist. This avoids re-discovering test infrastructure and architecture that `swt map` has already documented.
