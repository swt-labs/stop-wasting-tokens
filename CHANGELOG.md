...
- **`ad29101` fix(dashboard): retire InitScreen inline provider selector** — TopBar Provider menu is now the single source of truth. Pre-fix the inline selector showed multiple "Keychain" entries (label="Keychain" was used as the option display name) and could default to a non-authed provider, leaving the Initialize button greyed even when valid credentials existed. Removed 220 LOC (component logic + selectInitialProvider helper + 8 orphan CSS selectors + 5 retired tests). The Initialize button is gated on the globally-selected provider's authed state; if disabled, user opens the TopBar Provider menu to fix.
- **`0dd1d8a` fix(dashboard,runtime,shared): vendor communication + auth persistence bugs** — Two-bug fix in one commit. **Bug A** (CHAT_PROMPT_ERROR on back-to-back chat messages): Pi 0.74's `session.prompt(text)` requires `streamingBehavior='steer'|'followUp'` when an earlier prompt is still streaming; alpha.34 called `session.prompt(prompt)` with no options arg and Pi threw on the second message. Widened SwtSession.prompt to accept PromptOptions; chat route now passes `streamingBehavior='followUp'` so back-to-back user messages queue instead of erroring. **Bug B** (auth "forgotten" on restart): `buildSnapshot` in provider-auth.ts used config's `auth.<provider>.mode` field as the keychain probe key — when that field was null/absent, credentials stored under `swt:<provider>:oauth` would be missed. Snapshot now probes BOTH `'api_key'` and `'oauth'` when config doesn't pin a mode; first hit wins, the probe-resolved mode is surfaced. Downstream gates (Initialize button, chat credential resolver) now recognize keychain credentials correctly even when config.json's auth block is partially populated.
- **`e370089` style(dashboard): Log card background entirely black** — The shared `.panel` class applies a subtle green tint (`rgba(0, 255, 65, 0.04)`) to every panel. The Unified Log card inherited it via `class="panel unified-log"`, clashing with chat-user lines that already use `--deep-void`. Added `background: var(--deep-void)` to `.unified-log` for uniform terminal-black aesthetic. Theme-aware (light themes remap to a light page bg automatically). 1-line CSS edit.
- **`985aeac` fix(dashboard): Themes dropdown drops DOWN instead of opening to the right** — `.themes-dropdown` was missing the positioning block that `.github-dropdown` has (only `min-width: 240px` was set). Without `position: absolute; top: 100%; right: 0` anchoring it to the trigger, the popover rendered as an inline sibling block element to the right of the Themes button. Copied the GithubDropdown positioning block. CSS-only.
- **`1d1c97a` fix(dashboard): give first card a 'Plans' title** — The left-most card in the main grid rendered `<PhaseStepper>` (with its own "Phases" h2) when phases existed, falling back to an empty `<div>` (no title) otherwise. Per user direction, lifted the panel-header into App.tsx so the title is always visible — both empty-state and populated state now show "Plans".

### Features

- **`3dd2249` feat(dashboard): Model dropdown — picks per-provider model from Pi's ModelRegistry** — End-to-end feature delivered across all layers. New `GET /api/models` route projects Pi's `ModelRegistry.getAll()` into a metadata-only wire shape (`{ id, provider, name, contextWindow, reasoning }`). Runtime layer owns the Pi value-level import (Principle 1 invariant); dashboard server delegates to runtime; client renders a dropdown filtered by the currently-selected provider with checkmark + context-window display ('200K ctx' / '1M ctx' formatting). Selection persists to a new `config.model` field (top-level, takes priority over `model_profile` resolution when non-null). Mounted between Provider and Settings dropdowns in the TopBar chrome row. 12 files changed, +511 LOC across 3 NEW files (`runtime/src/models.ts`, `dashboard/src/server/routes/models.ts`, `dashboard/src/client/components/ModelDropdown.tsx`) and 9 modified.

**Test growth:** 2718 (alpha.34 baseline) → unchanged net (only chat-route assertion update from the streamingBehavior fix; no new tests added in alpha.35). **Regression:** 0 failed throughout. **D2 invariant preserved:** no provider-prompt edits in any commit. **Preflight (Gate A+B):** green at the v3.0.0-alpha.35 bump.

### Pre-publish housekeeping

The `docs/reference/config.mdx` was auto-regenerated by `pnpm docs:gen` after the new `model` field landed in Config.ts — matches Phase-01-of-milestone-22 precedent. No CHANGELOG entry needed for the docs regen itself (mechanical).

## Unreleased — milestone 22 (Settings Dropdown v2 — Flat Table + Profile Presets)

_Three-phase milestone replacing the dashboard's milestone-14 `Options ▼` dropdown (curated 10-knob block + Advanced recursive tree) with a single flat **`Settings ▼`** table — 24 rows in a 3-column layout (`Setting / Value / Description`), a `Profile ▾` sub-dropdown with 4 presets (Default / Turbo Mode / High-quality / Prototype), inline value editors (segmented buttons for enums, toggles for booleans, inputs for number/string, multi-select for arrays, toggle+input for the `max_uat_remediation_rounds` union), and a sticky `[Discard] [Save (N changes)]` footer. Preserves milestone-14's batched-staged-edit model (`pendingEdits` signal at parent, single `actions.applyConfigUpdate` POST, SSE `state.changed` broadcast). Brief: `a_non_production_files/archive/Option2.md` (863 lines, 15 locked decisions, 25 ACs). Archived 2026-05-19 as `22-settings-dropdown-v2-flat-table-profile-presets`; local tag `milestone/22-settings-dropdown-v2-flat-table-profile-presets` on commit `17aed91` (no npm publish — awaiting `v3.0.0-alpha.34` bundled release with milestone 21)._

**Cumulative impact:** 3 phases / 6 plans / 9 product commits (8 milestone-22 work + 1 bundled user-authored Themes Dropdown bonus feature) plus 1 archive-docs commit (`6f22807`). **Tests:** 2675 → 2718 (+43 net workspace-wide). **Test files:** 307 (Phase 02 added 3 new `.test.ts` files: `profiles.test.ts`, `settings-table.test.ts`, `profile-dropdown.test.ts`). **Regression suite:** 0 failed throughout. **Final preflight:** typecheck + lint + format:check + test + build all green (Gate A + B). **D2 invariant preserved throughout:** trivially preserved — milestone 22's work is L1 schema + L7 components/CSS, no provider-prompt edits.

### Phase 01 — SwtConfig schema extension (1 plan / 3 commits — `2413e1a` → `3292de0`)

- **`2413e1a` feat(core): extend SwtConfig with 15 settings-v2 fields + custom_profiles.** Edits `packages/core/src/config/Config.ts` (+39 LOC). New fields: `auto_commit`, `skill_suggestions`, `auto_install_skills`, `discovery_questions`, `discussion_mode` (enum `['questions','assumptions','auto']`), `context_compiler`, `visual_format` (enum `['unicode','ascii']`), `max_tasks_per_plan`, `branch_per_milestone`, `active_profile`, `qa_skip_agents`, `rolling_summary`, `require_phase_discussion`, `max_uat_remediation_rounds` (union `z.union([z.number().int().positive(), z.literal(false)])`), `caveman_style` (enum `['none','aggressive','extreme']`), plus `custom_profiles: z.record(z.string(), CustomProfileSchema)` for Future Work hook. All fields ship with `.default(...)` matching the brief's "current value" column → existing `.swt-planning/config.json` files parse cleanly via Zod's default chain, no migration script needed (backwards-compat Locked Decision #6). `docs/reference/config.mdx` auto-regenerated via `pnpm docs:gen` and amended into the same commit.
- **`3636333` test(core): defaults + enum vocab + backwards-compat for settings-v2 fields.** +124 LOC in `packages/core/test/config.test.ts`. 13 new `it()` cases under `describe('SwtConfig — settings-v2 fields')`: per-field defaults, enum vocab acceptance + rejection (negative case `discussion_mode: 'invalid'` throws `ConfigError`), union acceptance for `max_uat_remediation_rounds` (accepts `false` AND positive integers, rejects 0/negative), `custom_profiles` accepts a sample entry, backwards-compat via `ConfigSchema.parse({})`. Brings core/test/config.test.ts from 5 to 18 `it()` cases.
- **`3292de0` style(dashboard): drop redundant non-null assertion + prettier drift (Phase 01 release-discipline).** Gate B cleanup for pre-existing workspace-wide drift in `unified-log-helpers.ts:300` (TS no-unnecessary-type-assertion) + `unified-log-helpers.test.ts:307` (prettier). Same precedent as alpha.28 `a0a4dfd`, milestone 19 `97e0481`, milestone 21 `b755214`.

**4 drift-locks pinned pre-plan via deterministic orchestrator research** (5-milestone consecutive pattern — now 6-consecutive with milestone 22): (1) `active_profile: z.string()` open-string (NOT `z.enum(...)` — supports future custom profile IDs from `config.custom_profiles`; strict `ProfileId` narrowing happens at Phase 02 ProfileDropdown UI boundary). (2) Turbo profile `prefer_teams: 'never'` — brief Option2.md line 199 typo `'serialized'` is NOT in the existing enum `['auto','always','never']`; Phase 02 writes `'never'` in the Turbo profile values. (3) Intentional exclusions — `compaction_threshold`, `caveman_commit`, `caveman_review` are NOT in brief's 24-row target; they remain VBW-plugin-only config.json keys. (4) Schema-default backwards compat — no migration script needed.

### Phase 02 — Standalone components (3 plans / 4 commits — `47f1738` → `5b28bcd`)

- **`47f1738` feat(core): builtin profiles (Default/Turbo/Quality/Prototype) module.** NEW `packages/core/src/config/profiles.ts` (136 LOC) — exports `ProfileId`, `Profile`, `BUILTIN_PROFILES`, `PROFILE_IDS`. **Drift-lock 5 (CRITICAL, Scout caught):** brief specified `packages/shared/src/profiles.ts` (L0); profiles must reference `SwtConfig` from `@swt-labs/core` (L1) which is an upward L0→L1 import — forbidden by the dependency-layer invariant. Resolution: profiles module lives in `packages/core/src/config/profiles.ts` (L1, alongside the schema it depends on). NEW `packages/core/test/profiles.test.ts` (9 tests). Wave 1, no upstream deps.
- **`275cc25` feat(dashboard): SettingsTable + SettingsValueControl + setting-descriptions (settings-v2 components).** 7 files touched. NEW `SettingsTable.tsx` (148 LOC, props `{config, pendingEdits, onStage, onDiscardKey}`, exported pure helpers `formatValue` / `currentValueFor` / `isModified`), NEW `SettingsValueControl.tsx` (185 LOC, `inferControlType(key)` dispatcher — `'enum'`/`'boolean'`/`'number'`/`'array'`/`'string'`/`'union'`), NEW `setting-descriptions.ts` (76 LOC — `SETTING_DESCRIPTIONS` map + `SETTINGS_DISPLAY_ORDER` array, both 24 keys, one-to-one regression-tested). Extends `config-enum-vocab.ts` with 3 new enum vocabs (`discussion_mode`, `visual_format`, `caveman_style`), expands `SETTINGS_BOOLEAN_FIELDS` from 1 → 9 keys, adds `SETTINGS_NUMBER_FIELDS` + `SETTINGS_ARRAY_FIELDS`. Updates `settings-section.test.ts:226` assertion to match the expanded array (drift-lock 6 — test is slated for Phase 03 deletion but Phase 02 keeps it green). +138 LOC in `styles.css` — all Phase 02 selectors INCLUDING `.profile-dropdown*` per CSS-ownership Lead Option A (drift-lock 8). NEW `settings-table.test.ts` (24 tests).
- **`089f944` feat(dashboard): ProfileDropdown component + confirmation gate for unsaved changes.** NEW `ProfileDropdown.tsx` (146 LOC) — native `<select>` over `PROFILE_IDS` rendering `BUILTIN_PROFILES[id].name` labels, with inline confirmation row (`<Show when={confirming()}>`) intercepting profile switches when `hasPendingEdits=true` (Locked Decision #5 — prevents data-loss-by-design). Exported pure helpers `shouldConfirmSwitch` + `stageProfileValues` (the ONLY producer of `active_profile` in the staging path — `BUILTIN_PROFILES[id].values` deliberately does NOT contain `active_profile`; the helper sets it from `profile.id`). NEW `profile-dropdown.test.ts` (12 tests). Wave 2, ran as real team `vbw-phase-02` in parallel with Plan 02-02 — Agent tool with `team_name` + `name` parameters established real team semantics, both Devs committed concurrently with disjoint file sets verified pre-wave.
- **`5b28bcd` style(orchestration): prettier autofix for agents-md-loader (Phase 02 release-discipline).** Gate B cleanup for pre-existing drift in `packages/orchestration/src/context/agents-md-loader.ts`.

**4 drift-locks pinned pre-plan via deterministic orchestrator research:** (5) profiles module L0 → L1 (above). (6) `settings-section.test.ts:226` `SETTINGS_BOOLEAN_FIELDS` assertion expansion (Plan 02-02 Option A — update in-scope since the file is slated for Phase 03 deletion; equivalent regression assertion `SETTINGS_BOOLEAN_FIELDS.length === 9` survives in `settings-table.test.ts`). (7) Test convention `.test.ts` (NOT `.test.tsx` — workspace runs `environment: 'node'` with no JSX transform; pure-helper unit tests only, no `@solidjs/testing-library`). (8) CSS-ownership Lead Option A — Plan 02-02 owns ALL `styles.css` edits for Phase 02 (including `.profile-dropdown*` selectors referenced by Plan 02-03).

### Phase 03 — Integration & retirement (2 plans / 2 commits — `5885ac4` → `17aed91`, net **-970 LOC**)

- **`5885ac4` feat(dashboard): Themes dropdown — 8 user-selectable color schemes.** DEVN-04 **bundled commit** containing both user-authored Themes Dropdown feature AND Plan 03-01 deltas. Per `[feedback_pragmatic_over_protocol]` standing user direction. Plan 03-01 deltas: `R056 OptionsMenu.tsx → SettingsMenu.tsx` + `R058 options-menu.test.ts → settings-menu.test.ts` + DELETE 4 files (`SettingsSection.tsx`, `AdvancedConfigSection.tsx`, `settings-section.test.ts`, `advanced-config-section.test.ts` = ~1107 LOC removed) + modify `TopBar.tsx` (3 lines: import + JSX tag + trigger `Options ▾` → `Settings ▾`) + drop `SETTINGS_FIELD_ORDER` from `config-enum-vocab.ts`. Plus 3 critical drift-locks honored: (9) `mergeStagedConfig` re-homed from `SettingsSection.tsx` → `SettingsMenu.tsx` BEFORE deletion (CRITICAL — would have broken `handleSave` typecheck if reversed); (10) `handleStage` signature widened from `(key, value: string | boolean)` to `(key, value: unknown)` to accept arrays/numbers/false sentinel from `SettingsTable.onStage`; (11) `data-section="advanced"` structural assertion removed + 11 `stagePathEdit` pure-helper tests dropped (Advanced-tree helper retired alongside AdvancedConfigSection). Bundled bonus: 8 user-selectable themes (Default SWT terminal, Dark, Light, Solarized, Dracula, Nord, Monokai, Gruvbox), 33rd `theme: z.enum(THEMES).default('default')` field on SwtConfig, NEW `packages/dashboard/src/client/components/ThemesDropdown.tsx` + `themes-dropdown-helpers.ts`, 8 `:root[data-theme]` CSS blocks redefining 9 base vars per theme, `App.tsx` `createMemo`/`createEffect` on `state.tools.config.data?.config.theme` wired via existing `applyConfigUpdate` plumbing.
- **`17aed91` test(dashboard): full integration coverage for settings menu + profile flow.** +159 LOC in `settings-menu.test.ts` (43 → 48 tests). 5 new integration scenarios per brief §I.4: (1) picking profile stages all values, (2) manual edit overrides staged profile values, (3) save POSTs merged config including `active_profile`, (4) reload (snapshot replay) shows saved `active_profile`, (5) switching profile with pending edits triggers confirmation gate. Pure-helper composition tests — no DOM, no `@solidjs/testing-library` per drift-lock 7.

### Capabilities delivered

- **Single flat 24-row Settings table.** All settings at the same visual hierarchy — no curated/Advanced split. 3 columns: `Setting` (name) / `Value` (current, click-to-edit) / `Description` (one-line tooltip from `SETTING_DESCRIPTIONS`).
- **One-click profile presets.** Users pick Turbo / High-quality / Prototype to stage a coherent set of values (no auto-save — must click Save). Default profile resets to schema defaults. Switching with unsaved changes triggers confirmation gate.
- **Inline value editor.** Click a value → expand inline control row below (no popover): segmented buttons for enums, toggle for booleans, number input, multi-select for arrays, special toggle+number for the `max_uat_remediation_rounds` union. Esc collapses any open control.
- **Visible modified-row markers.** `data-modified="true"` + terminal-green left-border + dot indicator on rows with staged edits. Sticky `[Discard]` + `[Save (N changes)]` footer at the bottom of the dropdown.
- **Bonus: 8-theme dashboard skinning.** User-authored Themes Dropdown ships alongside in commit `5885ac4`. Theme persisted via existing `applyConfigUpdate` POST; live re-skinning via `<html data-theme>` switching; per-component CSS already uses `var(--name)` tokens throughout, no per-component changes needed.

### 11 drift-locks held across milestone (5-milestone pre-plan-research pattern now 6-consecutive: 17/19/20/21/22 + this one)

Phase 01: active_profile open string · Turbo prefer_teams 'never' · intentional exclusions · schema-default backwards compat. Phase 02: profiles module L0 → L1 · settings-section.test.ts:226 expansion · .test.ts convention · CSS-ownership Plan 02-02. Phase 03: mergeStagedConfig re-homing · handleStage signature widening · data-section="advanced" assertion removal + stagePathEdit test deletions.

### DEVN-class deviations

- **DEVN-02 (Phase 01):** `docs/reference/config.mdx` auto-regen amended into T01 commit (3-file footprint vs 2-file plan estimate).
- **DEVN-02 (Phase 01):** Workspace-wide lint/format drift in `unified-log-helpers.{ts,test.ts}` fixed in separate `3292de0` style commit (Gate B precedent).
- **DEVN-02 (Phase 02):** prettier-autofix for `orchestration/agents-md-loader.ts` in separate `5b28bcd` style commit (Gate B precedent).
- **DEVN-04 (Phase 03):** Plan 03-01 deltas bundled with user-authored Themes Dropdown feature in commit `5885ac4`. Atomic-commit invariant violated; content correct; per `[feedback_pragmatic_over_protocol]` user intent honored. No corrective rebase per I7 do-no-harm rule.

### 6-milestone-recurring slug-script bug — ESCALATION CANDIDATE

`derive-milestone-slug.sh` outputs word-salad gibberish for the 6th consecutive milestone (17/18/19/20/21/22). Today's script output: `21-25-rows-in-a-3-column-layout-setting-value-description` (off-by-one milestone number + parsing CONTEXT.md prose). Workaround per CLAUDE.md: parse `**Milestone slug:**` from ROADMAP.md via grep. Plugin needs a dedicated fix in a future milestone.

### Bonus: user-authored agent_card Wave 1 (commits `fb99a70`, `2fc0b9d`, `8ddd244`, `a817337`)

User-authored post-milestone-22 work building toward `a_non_production_files/agent_card.md`:

- **`fb99a70` refactor(dashboard): extract compactTokens + shortModelLabel to lib/model-helpers.ts.** NEW `packages/dashboard/src/client/lib/model-helpers.ts` + tests.
- **`2fc0b9d` fix(dashboard): render model, cache tokens, sub_session_id, pid in agents table.** Wire model name + cache-token columns + sub-session-id + PID into the ActiveAgents pane table view.
- **`8ddd244` fix(dashboard): artifact link + token split + relative time in agent history.** Agent history view polish.
- **`a817337` style(dashboard): prettier + import-order fixups after agent_card Wave 1.** Gate B precedent applied to bundle these out-of-VBW commits with workspace-wide format check before alpha.34.

---

## Unreleased — milestone 21 (OpenAI Codex OAuth — ChatGPT-subscription auth parity)

_Two-phase milestone closing the OpenAI Codex OAuth feature-parity gap with Anthropic OAuth. Trigger: user one-liner citing https://developers.openai.com/codex/cli — "OpenAI explicitly says OAuth is a valid way to auth into Codex for ChatGPT users, so please apply." Archived 2026-05-19 as `21-openai-codex-oauth-chatgpt-subscription-auth`; local tag `milestone/21-openai-codex-oauth-chatgpt-subscription-auth` on commit `b755214` (no npm publish — awaiting `v3.0.0-alpha.34` bundled release with milestone 22)._

**Cumulative impact:** 2 phases / 2 plans / 5 product commits (`228571f` → `b755214`) plus 1 archive-docs commit (`48ae337`). **Tests:** 2659 → 2681 (+22 net). **Regression suite:** 115/27/0 (matches milestones 17-20 baseline). **D2 invariant preserved (byte-identical to milestone 19):** Anthropic OAuth advisory at `init.ts:285-298`, Anthropic augmenter branch at `cook.ts:1156-1223`, Anthropic headline `Anthropic says: Add more at claude.ai/settings/usage and keep going.` — all unchanged.

### Phase 01 — Pi-ai OAuth audit + runtime wiring (2 commits — `228571f`, `cfff42c`)

Pi-ai 0.74.0 ships native OpenAI Codex OAuth via `dist/utils/oauth/openai-codex.{js,d.ts}` exporting `loginOpenAICodex` + `openaiCodexOAuthProvider`. Token client_id: `app_EMoamEEZ73f0CkXaXp7hrann` (OpenAI's published Codex CLI client). Scopes: `openid profile email offline_access`.

**DRIFT-1 (CRITICAL, caught pre-plan via orchestrator-research):** `openaiCodexOAuthProvider.id === "openai-codex"`, NOT SWT's `"openai"` — `getOAuthProvider("openai") === undefined` would 400 every OAuth start. **Resolution:** NEW `packages/runtime/src/credentials/oauth/provider-id-map.ts` (51 LOC) exporting `mapToOAuthProviderId(swtProviderId): string` with table-driven `{openai → openai-codex}` + identity fallback. Dashboard route at `provider-auth-oauth.ts:214` maps ONCE; `mappedProvider` flows into ONLY `getOAuthProvider(mappedProvider)` at L221 + `runOAuthLoginFlow({ provider: mappedProvider, ... })` at L241 — every other use of `provider` (events, credentialRef `swt:openai:oauth`, `auth.openai` config, response body) keeps the SWT canonical id end-to-end. +12 tests (10 mapping/uniformity + 2 route smoke).

### Phase 02 — Provider menu UI + InitScreen advisory + augmentSpawnError parity (3 commits — `d56ff01`, `6b39737`, `b755214`)

**DRIFT-1 (CRITICAL, caught pre-plan):** `ProviderAuthPanel.tsx:173` `OAUTH_PROVIDERS` array contained pi-ai's `'openai-codex'` instead of SWT's `'openai'` — `isOAuthProvider('openai') === false` would have disabled the OAuth radio for OpenAI users (same root cause as Phase 01's drift, leaked into UI surface). Single-token array-literal edit fixes it, locked by 2 regression tests.

InitScreen advisory sibling at `init.ts:301-309` (after the existing Anthropic block at L285-298). URL: `https://platform.openai.com/account/billing/overview`. `augmentSpawnError` OpenAI branch at `cook.ts:1225-1281` below Anthropic's `/out of extra usage/i` block. Pattern: `/rate_limit_exceeded|quota.*exceed|insufficient_quota|billing/i` AND `context?.provider === 'openai'` (provider+pattern gate, disjoint from Anthropic). Three sub-branches: oauth / api_key / no-context-backwards-compat. Headline: `OpenAI says: Check your usage at platform.openai.com/account/billing/overview and keep going.`. +10 tests (2 OAUTH_PROVIDERS regression-locks + 3 sibling describes + 2 cross-provider disjointness assertions).

### Capabilities delivered

- **ChatGPT subscription users can OAuth into SWT.** Pick OpenAI as provider → OAuth radio enables → flow start emits auth_url → user authorizes at auth.openai.com → callback persists credentials. Same UX shape as Anthropic OAuth flow.
- **Quota-exhaustion failure message points to OpenAI billing page.** When the Codex CLI OAuth client hits rate limits / quota exhaustion, the spawn-error augmenter shows `OpenAI says: Check your usage at platform.openai.com/account/billing/overview and keep going.` — parallel to the Anthropic `claude.ai/settings/usage` message.

### 3 deviations recorded

(1) Phase 01 root-barrel re-export at `runtime/src/index.ts` (+2 lines) — required for `@swt-labs/runtime` import path resolution from dashboard. (2) Phase 01 sibling test mock-factory tweak in `provider-auth-oauth-route.test.ts` (+13 LOC) — required because pre-existing test mocks `@swt-labs/runtime`. (3) Phase 02 separate atomic `style(dashboard):` prettier commit `b755214` — Gate B precedent (alpha.28 `a0a4dfd`, milestone 19 `97e0481`, milestone 20 `b174a15`).
...
**Cumulative impact:** 4 phases / 4 plans / 9 product commits (`f1cb88d` → `1bc3ef4`) plus 1 archive-docs commit (`821848c`). **Tests: 2639 passed / 67 skipped / 0 failed** (+41 from milestone-18's 2598 baseline: 6 Phase 01 + 2 Phase 02 + 22 Phase 03 + 9 Phase 04 + edge cases). **Regression suite:** 115 / 27 / 0 (matches milestone-18 baseline exactly — zero new failures). **D2 invariant preserved throughout:** trivially preserved — milestone 19's work is runtime trace-sink + cli failure-text + dashboard widgets, no provider-prompt edits.

### Phase 01 — Foundation: Pi-event trace sink + init stdout pipe — `f1cb88d` → `97e0481`, 3 commits

`packages/orchestration/src/spawn-agent.ts` ports the alpha.23 trace-sink pattern from `spawn-orchestrator-session.ts:359-430` byte-for-byte (72 LOC). `SpawnAgentOptions` gains optional `traceWriter?: ((line: string) => void) | null` (default: `process.stderr.write`, respects `SWT_NO_LLM_TRACE=1`). Buffers `MESSAGE_DELTA` per turn → flushes as `[llm turn N] <text>` on `TASK_TOKEN_USAGE` / `TASK_ERROR` (truncated at 2000 chars). `TOOL_CALL` flushes pending message + emits `[tool] toolName` inline. Defensive try/catch around writer (subscriber survives throwing writer across events). `packages/dashboard/src/server/routes/init.ts:201` changes stdio from `['ignore', 'ignore', 'pipe']` to `['ignore', 'pipe', 'pipe']`; duplicated stderr-mirror block for stdout → `log.append` SSE events with `channel: 'stdout'` (no ring buffer — the stderr ring exists only for the `init.error` envelope). Every `spawnAgent` callsite now emits visible Pi activity for free; the alpha.23 fix that only covered orchestrator-session is now applied to the spawn-agent path that init takes. 5 new spawn-agent tests + 1 init-route stdout test. Includes 1 release-discipline Gate B fix-in-milestone commit (`97e0481` prettier format on CLAUDE.md for a pre-existing single-character drift introduced in milestone 18's archive commit — kept in its own atomic `style()` commit so T01/T02 product changes remain auditable).

### Phase 02 — Failure UX: augmentSpawnError context + branching — `75d9dc9` → `4b6bc75`, 3 commits

`packages/cli/src/commands/cook.ts` exports new `AugmentSpawnErrorContext` interface (`{authMode?: 'oauth' | 'api_key'; provider?: string}`). `augmentSpawnError` extends additively to `(rawSummary, context?)`. Inside the `/out of extra usage/i` arm, three branches: **api_key** (Case B only — no `9d1c250a-e61b-44d9-88ed-5944d1962f5e`, no Provider menu, no `ANTHROPIC_API_KEY`, no allowlist text; the augmenter MUST NOT mention these when the user is already on an API key); **oauth** (URL leads, allowlist hypothesis demoted to "If your account shows healthy remaining quota, this is probably what's happening" secondary cause, both workarounds priority-ordered below the top-up advice); **no-context** (byte-identical to today's output for backwards-compat — AC-12). All three branches lead with `Anthropic says: Add more at claude.ai/settings/usage and keep going.` `request_id` extraction via defensive regex `/"request_id":"([^"]+)"/`. `init.ts:346` passes `{authMode: resolvedAuth?.resolvedCredential.authMode, provider: resolvedAuth?.provider}`. `cook.ts:3401` resolves context via `await resolveSpawnCredential(fallbackResult.providerUsed, config.auth)` (Option A — the helper is already imported at cook.ts:98 and used at L3196 in the `onResolveCredential` callback; the enclosing `runMode` function is async so `await` is legal). Pre-spawn OAuth advisory at `init.ts:285-291` rewritten per Decision #17: leads with `https://claude.ai/settings/usage`, demotes the allowlist hypothesis to conditional on healthy quota. `cook-error-augmenter.test.ts` replaces the single `Anthropic Max-plan OAuth third-party billing pool` describe with 3 sibling describes (`api_key context`, `oauth context`, `no context (backwards-compat)`); AC-12 enforced via explicit `.toBe(BASELINE)` literal rather than `.toMatchSnapshot()` so PRs review the wording and snapshot-update accidents can't silently regress. 6 → 8 tests.

### Phase 03 — Live progress block: dashboard reducer + InitScreen widget — `6bfc4f4` → `c72d118`, 2 commits

`packages/dashboard/src/client/state/dashboard-store.ts` extends `InitSessionState` interface at L84-95 with `lastMessage?: string;`. `log.append` reducer at L1506 sets `state.initSession.lastMessage` to `evt.line` when `state.initSession !== null && state.initSession.status === 'detecting'`. Optimistic `init.start` write at L1993 initializes `lastMessage: undefined` (Scout-flagged field-shape invariant). One-line temporal-correlation invariant comment documents: log.append carries no `session_id` (events.ts:44-49); init + cook do not overlap; if cook-during-init flow emerges, this needs a discriminator. `packages/dashboard/src/client/components/InitScreen.tsx` adds exported `classifyInitLine` pure helper (L50) + inline `toolFriendlyLabel` map (L14): `[tool] X` → `{friendly} (X)` via small Pi-tool-name map (Read/Grep/Glob → "reading project files…", Write/Edit → "writing files…", Bash → "running commands…", default → bare tool name); `[llm turn N] <text>` → `thinking… "{first 80 chars}"`; three CLI contract-line prefixes → their friendly forms; anything else → render raw; undefined/empty → `detecting stack…`. Elapsed counter via `createSignal(tick) + setInterval(1000) + onCleanup` (matches existing Solid primitives in the file). `<Show when={isBusy()}><div class="init-progress" role="status" aria-live="polite">◆ {displayLabel()} ({elapsed()}s)</div></Show>` placed above `.init-actions` at L296. Long-running fallback at `elapsed > 120` swaps to `still working — large repos can take 3-5 min on first run`. `.init-progress` CSS at styles.css:642 uses `JetBrains Mono` + `--slate-muted` + `padding: 8px 0` — no spinner/throbber/animation per Decision-NOT-to-do (the `◆` glyph + ticking elapsed counter IS the animation). **NEW FILE** `packages/dashboard/test/init-screen-helpers.test.ts` (110 LOC, 18 cases covering `classifyInitLine` truth table). +4 reducer cases in `dashboard-store.test.ts`. 22 net new tests.

### Phase 04 — Provider selector + auth-status indicator — `1bc3ef4`, 1 commit

Two new exported pure helpers at `InitScreen.tsx:30-58`: **`selectInitialProvider(credentials, selectedProviderFromSnapshot)`** returns first-config-match → first-authed → first-overall → null (matches by `c.provider === selectedProviderFromSnapshot` — provider-only key, not composite); **`computeProviderStatus(credential)`** returns `'green'` when `configured === true && mode !== null`, `'red'` otherwise, `'empty'` when `credential === null`. Provider dropdown + status indicator button render above PROJECT NAME when `credentials.length > 0`; empty-state `<div class="provider-empty">No providers configured — open the Provider menu →</div>` renders otherwise. Each dropdown option labels as `{c.label ?? c.provider} ({c.mode ?? 'no-mode'})`. Status button toggles `class="provider-status provider-status--{ok|missing}"` with content `✓`/`✗` and a tooltip. onClick (status button or empty-state link) dispatches `props.onOpenProviderMenu()` → `openProviderMenu` action — same surface as TopBar.tsx's top-right Provider menu. `<select>` onChange dispatches `props.onSelectProvider({provider, mode})` → `applyProviderAuthUpdate(body)` (wraps `postProviderAuth` + optimistic-update + error-push). Initialize button gating: `disabled = isBusy() || computeProviderStatus(selected()) !== 'green'`. Tooltip when disabled-from-provider: `Provider credentials needed — click the red indicator to add them.` Cross-surface sync via reactive `state.tools.providerAuth.data` — change in either surface triggers SSE broadcast → both re-render. `App.tsx` threads 3 new props on the `<InitScreen>` instantiation. CSS at styles.css:636+: `.provider-selector` (flex), `.provider-status` (32x32 button with terminal-aesthetic border), `.provider-status--ok` / `.provider-status--missing` using `--terminal-green` / `--danger-red` variables, `.provider-empty` empty-state container. **6 brief drift corrections applied** (Scout's research flagged 3, Lead's pre-planning grep surfaced 3 more): P1 schema (`configured + mode` not `status`), P2 state path (`state.tools.providerAuth.data.statuses` not `state.providerAuth.credentials`), P3 dispatcher (`applyProviderAuthUpdate` not the brief's hypothetical `setActiveProvider` — which doesn't exist), P4 SoT field (`selected_provider` provider-only, not `provider:mode` composite), P5 CSS var (`--terminal-green`, not `--success-green`), P6 wiring (App.tsx instantiation must thread the 3 new props since Phase 03 left InitScreen at 4 props). 9 new pure-function tests (5 selectInitialProvider + 4 computeProviderStatus) appended to Phase 03's `init-screen-helpers.test.ts`.

### Capabilities delivered (problem-by-problem)

**Problem 1 — Silent init RESOLVED.** Every `spawnAgent` callsite now emits Pi-event activity via the trace-sink writer. Init subprocess stdout AND stderr both flow to the dashboard's `log.append` SSE channel. InitScreen renders a live progress block above the Initialize button that classifies the most recent line, ticks an elapsed counter every 1s, and swaps to "still working" copy after 120s.

**Problem 2 — augmentSpawnError misclassification RESOLVED.** Anthropic "out of extra usage" now branches correctly on `authMode`: api_key gets Case B only (no allowlist confusion), oauth gets both causes priority-ordered with the pivot sentence ("If your account shows healthy remaining quota..."), no-context preserves today's behavior byte-identical. All branches headline with the actionable URL.

**Problem 3 — Invisible provider auth RESOLVED.** InitScreen renders a provider dropdown + auth-status indicator above PROJECT NAME, sharing state with the existing top-right Provider menu via reactive `state.tools.providerAuth.data`. Initialize button disabled when status is red or empty, with tooltip. Status button click and empty-state link both dispatch the existing `openProviderMenu` action — zero duplicated auth UI.

### Process improvements memorialized

- **Spot-check + banner closeout pattern** (user-elected, established across milestones 18 + 19, all 4 phases of each). Orchestrator verifies: claimed commits + preflight green workspace-wide + out-of-scope files clean + smoke test PASS + working-tree clean, then presents banner. No formal `*-VERIFICATION.md` artifacts. Archive mode's 7-point audit treats this as WARN (allows proceed). Full QA agent path remains available when explicitly requested.
- **6-drift adaptation pattern (Phase 04):** When a milestone brief is fully baked but the brief's API references drift from actual codebase shape, Scout's research surfaces the drifts and Lead's pre-planning grep confirms additional ones. Lead embeds the corrected API references inline in the plan; Dev follows the plan, not the brief. Composes cleanly with `[feedback_pragmatic_over_protocol]` while honoring `[feedback_optimal_elegant_no_debt]`.

### Carry-overs out of milestone 19

- **DEVN-05** — `packages/dashboard/src/client/components/Popover.tsx:138` TS2322 ARIA-role-union error. Accepted-process-exception now across milestones 13/14/15/16/17/18/19. Seven-milestone carry-over needing a dedicated phase to revisit ARIA-role typing.
- **DEVN-PHASE-06-DRIFT-CI-DEFERRED** (from milestone 17) — CI workflow step to run `pnpm gen:apply-patch-parser` + `git diff --exit-code` on the parser file would close the Lark drift loop end-to-end.
- **`.vbw-planning/` migration fallback** (from milestone 18) — `phase-detect.ts:189` retains migration fallback as lone Locked Decision #6 exception. Deferred to Milestone H-02.
- **Phase 04 Solid component test for provider selector** — deferred during execution (pure-function helpers covered; component-test for dropdown render shape + cross-surface sync is follow-up patch).

## Unreleased — milestone 18 (Runtime Preconditions & Greenfield Routability)

_Four-phase milestone enforcing Locked Decision #6 ("no silent fallbacks") uniformly across SWT's runtime path, triggered by two 2026-05-18 user-reported alpha.31 incidents (dashboard 404 on greenfield + tarball ships 3/166 scripts). Archived 2026-05-19 as `18-runtime-preconditions-and-greenfield-routability`; local tag `milestone/18-runtime-preconditions-and-greenfield-routability` (no npm publish — awaiting `v3.0.0-alpha.33` bundled release with milestone 19)._

**Cumulative impact:** 4 phases / 4 plans / 20 product commits (`9b4bd93` → `797fbef`) plus 1 archive-docs commit (`fbe25df`). **Tests: 2598 passed / 67 skipped / 0 failed** (+4 from milestone-17's 2594 baseline — Phase 04 truth-table tests; Phases 01-03 added net-zero tests, route-handler additions covered by existing snapshot infra and tarball-shape script + fallback sweep are infrastructure-only). **Regression suite:** 115 / 27 / 0 (matches milestone-17 baseline exactly — zero new failures). **D2 invariant preserved throughout:** trivially preserved — milestone 18's work is runtime-precondition assertions + infrastructure, no provider-prompt edits.
...
Triggered by 2026-05-18 user report `weird-vibes.md`: published `stop-wasting-tokens@3.0.0-alpha.31` tarball shipped 3 of 166 scripts and 0 of 15 `references/` files. `package.json` `files[]` extended from 16 to 13 entries (7 superseded individual scripts removed, 4 dirs added: `references/`, `templates/`, `scripts/`, `config/`). **DEVN-02 discovery:** Scout assumed root `.npmignore` would carve out `scripts/internal/` from `files[]`. Per npm 11 + npm/cli#6221, npm/cli#4069, root `.npmignore` does NOT override `files[]`. Required 6th corrective commit (`2e6ad6c`) moving `.npmignore` from root to NESTED location at `scripts/.npmignore` (which DOES override `files[]`). New `scripts/check-tarball-shape.mjs` (Node ESM) runs `npm pack --dry-run --json` and asserts 10 sentinel-file paths (e.g., `references/codex/gpt-5.2-codex_instructions_template.md`, `templates/cook/...`, `scripts/spawn-snapshot.ts`) — hard-exits non-zero with actionable message on miss. `pnpm release:preflight` extended with `check:tarball-shape` step (Release Discipline §A: same gates that gate CI must gate the local release path).

### Phase 03 — Silent-fallback removal sweep across commands + runtime — G-03, 5 commits

27 silent-fallback patterns converted to hard-error across 5 `commands/*.md` files (Pattern A: 5 NORM_SCRIPT, Pattern B: 22 PG_SCRIPT). `packages/cli/src/commands/verify.ts:457` `const installRoot = resolveInstallRoot()` (was `?? io.cwd` bug). `packages/runtime/src/env.ts` audited — already conforms to Locked Decision #6, no changes needed (commit `b486ad6`). Each precondition check that previously degraded silently now exits non-zero with actionable message at first occurrence.

### Phase 04 — Pi-extension registration assertion — G-04, 2 commits

`packages/runtime/src/session.ts` (+29/-2): the two `materializeExtensionsToCustomTools(opts.extensionFactories)` call sites (formerly at lines 198 + 236) hoisted to a single shared site after `buildPiResourceLoader` at L165. New 13-line guard throws `"SWT: Pi extension registration check failed — N factory(s) supplied but zero customTools materialized. Expected registration via spawn-orchestrator-session.ts. This likely means the install is incomplete. Run 'swt doctor' for diagnostics."` when `opts.extensionFactories?.length > 0 && customTools.length === 0`. **Critical scope correction during research:** `swt_task` does NOT exist as a SWT custom tool — it's Pi's native built-in `Task` from `createCodingTools()`. Pi 0.74 has no registered-tools query API, so the assertion lives at the INPUT side (materializer's return value) — the only assertable boundary. `packages/runtime/test/session.test.ts` (+154/-1): 4 new `vi.doMock` truth-table tests. Execution-time surprise: `vi.resetModules()` at start of `loadCreateSessionWithMock` helper required to defeat top-of-file static import caching of Pi module graph via re-exports of `session.js`.
...
- **Spot-check + banner closeout pattern** (user-elected for all 4 phases of milestone 18). Lighter than full QA agent — orchestrator verifies Dev's claimed commits exist + preflight green + out-of-scope guards clean + smoke test PASS + working-tree clean, then presents banner. No formal `*-VERIFICATION.md` artifacts.
- **`derive-milestone-slug.sh` bug bypass** — script returns garbled slug. Workaround: use `**Milestone slug:**` value from ROADMAP.md frontmatter directly per `[feedback_pragmatic_over_protocol]`. Validated on milestones 17 + 18.
- **`git mv` failure mode for gitignored planning artifacts** — `.vbw-planning/` is fully gitignored; `git mv` fails with "fatal: not under version control". Use plain `mv` for archive operations. Memorialize as part of the gitignored-planning architecture.

### Carry-overs out of milestone 18

- **DEVN-05** Popover.tsx:138 — 6-milestone exception, accepted-process-exception across 13/14/15/16/17/18.
- **DEVN-PHASE-06-DRIFT-CI-DEFERRED** — from milestone 17, Lark drift CI step.
- **`.vbw-planning/` migration fallback at `phase-detect.ts:189`** — lone Locked Decision #6 exception, deferred to Milestone H-02.

## Unreleased — milestone 17 (Codex CLI Parity via ProviderTuningPack)

_Six-phase milestone closing the gap between SWT-on-Codex sessions and standalone Codex CLI by extending Pi 0.74 at the SWT/Pi-extension layer (Option A). New provider-agnostic `ProviderTuningPack` interface (7 fields: `providerId`, `displayName`, `resolveOverlay`, `customExtensions`, `contextFiles`, `extractUsage`, `upstreamSources`) becomes the single composition point through which `AnthropicViaPiPack` and `CodexViaOverlayPack` plug in. Archived 2026-05-18 as `17-codex-cli-parity-via-provider-tuning-pack`; local tag `milestone/17-codex-cli-parity-via-provider-tuning-pack` (no npm publish — awaiting `v3.0.0-alpha.32` version bump)._

**Cumulative impact:** 6 phases / 6 plans / 43 product commits (`529089b` → `1ecbace`) plus 1 archive-docs commit (`007454c`). **Tests: 2581 passed / 67 skipped / 0 failed** (+38 from milestone-16's 2543 baseline: +18 Phase 04 LogEntry/dashboard, +6 Phase 05 CLI verb, +14 Phase 06 cassette-replay). **Regression suite:** 115 / 27 / 0 (matches milestone-16 baseline exactly — zero new failures). **D2 invariant preserved throughout:** Anthropic spawn-snapshot byte-identical across all 8 role-keys from Phase 04 close (sha256 `be2d691e89b12…`); full snapshot byte-identical from Phase 05 close (sha256 `8ce8083539c80…`) through Phase 06.

### Phase 01 — ProviderTuningPack foundation (no-op refactor) — `529089b` → `e90735c`, 4 commits

Strict-no-op refactor establishing the `ProviderTuningPack` abstraction. `packages/orchestration/src/provider-tuning-pack.ts` defines the 7-field interface (with optional `contentHash?: string` on `UpstreamSource`). `packages/orchestration/src/packs/{anthropic-via-pi,codex-via-overlay}.ts` implement the two packs. `packages/orchestration/src/packs/registry.ts` exposes `getProviderTuningPack(providerId)`. `packages/orchestration/src/spawn-agent.ts` exports `APPLY_PATCH_ELIGIBLE_ROLES = {lead, dev, qa, debugger, docs}` as the authoritative code-editing-role constant (consumed by Phase 04). `scripts/spawn-snapshot.ts` (NEW, gitignore-carved) is the D2 invariant fixture tool: dumps per-role/per-provider spawn config as JSON for byte-comparison. Anthropic + OpenAI snapshots byte-identical to pre-Phase-1 baseline by construction.

### Phase 02 — OpenAI overlays canonical refresh — `3c8cb46` → `744b1f7`, 14 commits + R01

Refreshes 7 `provider_overlays/*-openai.md` files against the canonical Codex template vendored at `references/codex/gpt-5.2-codex_instructions_template.md` (sha256 `492a212d8a23b…`). D8 selective-adoption pattern: overlay rewrites adopt canonical-aligned formatting (backticked `apply_patch`, hyphenated `single-file`, parenthetical clarifications) while preserving SWT role conventions. `.prettierignore` gains `references/codex/` carve-out so prettier's auto-format never mutates the canonical body's sha256. R01 force-advanced after pure plan-amendment round (memorialized gate-quirk: `paths_cover_required_original_plan_artifacts` can't validate gitignored-planning plan-amendment rounds — workaround documented in memory `project_vbw_gate_gitignored_planning_gap`).

### Phase 03 — AGENTS.md hierarchical loader — `1bb9ed2` → `0fe4ca8`, 10 commits + R01 code-fix

`packages/orchestration/src/context/agents-md-loader.ts` (NEW) implements walk-up (`.git`-ancestor → cwd) discovery and `AGENTS.override.md` REPLACE semantics, mirroring Codex's `agents_md.rs` (sha256 `ce3201eaee6cd…`). R01 surfaced GATE-07/15 real FAIL: Phase 03 wired config-field plumbing but didn't reach Pi's session-start prompt construction. R01 code-fix landed `buildPiResourceLoader(opts)` at `packages/runtime/src/session.ts` (constructs `new DefaultResourceLoader({cwd, agentDir, noContextFiles: true, systemPrompt?, agentsFilesOverride})`); both `createAgentSession` call sites carry the loader via conditional-spread `...(resourceLoader !== undefined ? { resourceLoader } : {})`. `packages/shared/src/types/session.ts` extended with `systemPrompt?: string` + `contextFiles?: readonly string[]` (DEVN-01 plan-amendment: plan listed `runtime/src/types.ts` but that's been a thin re-export from `@swt-labs/shared` since PR-04). 4 new unit tests at `session-pi-bridge.test.ts` assert the wiring contract.

### Phase 04 — `update_plan` tool shim — `818a5e6` → `89420eb`, 4 commits (clean-PASS pattern discovery)

`packages/runtime/src/extensions/update-plan-tool.ts` (NEW) ships `buildUpdatePlanExtension` mirroring `buildApplyPatchExtension`'s `(opts?) => (pi) => void` shape. Tool name `"update_plan"`, description verbatim `"Updates the task plan. At most one step can be in_progress at a time."`, Zod `.strict()` schema, status enum `pending|in_progress|completed` — all byte-for-byte from Codex's `plan_tool.rs`. On execute: `pi.appendEntry('cook.plan_update', parsedArgs)`. 10th `cook-plan-update` LogEntry variant added to `packages/shared/src/types/log-entry.ts`; `CookEvent` wire type extended; dashboard render at `packages/dashboard/src/client/components/unified-log-helpers.ts` produces `[x] / [~] / [ ]` indicators on monospace `|`-separated lines; `UnifiedLogPanel.tsx`/`UnifiedLogRow` gates the new render branch; reducer uses `setState('unifiedLog', idx, entry)` for replace-in-place semantics. `CodexViaOverlayPack.customExtensions(role)` returns `[applyPatchExtension, updatePlanExtension]` for `APPLY_PATCH_ELIGIBLE_ROLES` (DEVN-PHASE-04-ROLE-GATING plan-amendment — brief said `{Dev, Architect, Debugger}` but Phase 01's constant is `{lead, dev, qa, debugger, docs}`; architect is read-only and stays excluded). 12 new runtime test cases + 7 new dashboard helper tests. **Clean-PASS pattern memorialized for future phases:** pre-registered deviations live in PLAN frontmatter ONLY; SUMMARY `deviations: []`; no body `## Deviations` section.

### Phase 05 — Upstream drift automation refactor — `df1b930` → `c3f74b5`, 6 commits

`packages/cli/src/commands/provider-tuning-sources.ts` (NEW) ships `swt provider-tuning-sources` CLI verb emitting JSON envelope `{schema: "v1", generated_at: ISO8601, sources: [{source: UpstreamSource, packId, packDisplayName, method: "upstreamSources"}, ...]}`. `packages/orchestration/src/index.ts` extended with 4 new exports (`getProviderTuningPack`, `getAllPacks`, `UpstreamSource`, `ProviderTuningPack` types — DEVN-PHASE-05-ORCHESTRATION-EXPORTS plan-amendment). `AnthropicViaPiPack.upstreamSources()` populated with Anthropic SDK npm tarball entry (DEVN-PHASE-05-ANTHROPIC-SDK-SOURCE — brief AC#2 required SDK drift detection after refactor). `CodexViaOverlayPack.upstreamSources()` populated with canonical template + `agents_md.rs` + `apply_patch.lark` entries. `scripts/audit-upstream-prompts.sh` refactored to consume `swt provider-tuning-sources` JSON via `jq`; ZERO hardcoded URLs remain (CODEX_URL, CLAUDE_SDK_PKG/FILE constants deleted); tempdir + EXIT-trap license-hygiene mechanism preserved. `.github/workflows/upstream-prompt-audit.yml` refactored in-place (DEVN-PHASE-05-WORKFLOW-IN-PLACE — no parallel workflows): name renamed to `"Upstream Drift"`, cron set to `0 6 * * 1` (weekly Monday 06:00 UTC), pnpm/action-setup@v4 + `pnpm install --frozen-lockfile` + `pnpm build` steps added; Issue body cites source description + pack_id + method + current/expected hashes + URL. Includes 2 release-discipline fix-in-milestone commits (`824bb42` lint void-mark on 5 sync handler tests; `c3f74b5` `pnpm docs:gen` regen of `docs/reference/cli.mdx` for `test/docs/drift.test.ts`).

### Phase 06 — Lark-derived `apply_patch` parser — `cbd6c8f` → `1ecbace`, 5 commits + R0 direct-fix

Replaces the 365-LOC hand-rolled `packages/runtime/src/extensions/apply-patch-parser.ts` with a parser derived from upstream Codex Lark grammar. **D7 single-commit replacement honored:** `64f3442` overwrites the parser body in place — no `.bak`, no parallel file, no feature flag. `references/codex/apply_patch.lark` vendored (sha256 `d6367f4826ed…` matches Phase 5's pin + live URL). `scripts/codegen/apply-patch-from-lark.ts` (NEW, gitignore-carved) generates the parser deterministically — re-running produces byte-identical output. `pnpm gen:apply-patch-parser` npm script registered for future CI drift-check wiring. `scripts/codegen/capture-apply-patch-fixtures.ts` (NEW) captures parser output across all 14 fixtures; `.vbw-planning/phases/06-lark-derived-apply-patch-parser/{old-parser-baseline,new-parser-output}.json` both sha256 `05e5136c748e8…` — **cassette-replay byte-identity** across the parser swap (Phase 06 AC#2). Public API (`parseApplyPatch`, `ApplyPatchResult`, `FileOp`, `Hunk`, `ChangeLine`) unchanged; `apply-patch-tool.ts` consumer untouched. 14 existing parser tests pass without test edits. New `packages/runtime/test/extensions/apply-patch-parser-cassette-replay.test.ts` (14 cases) guards future regenerations. Path-(a) selected over path-(b) (DEVN-PHASE-06-PATH-A-NOT-B — Scout confirmed no JS/TS reference parser exists in `openai/codex` repo; `lark-js` npm unmaintained Feb 2024). Hard-codes 5 verbatim error-message strings (`CRLF`, `[Aa]bsolute`, `at least one|zero`, `Begin Patch`, `+`) since the Lark grammar doesn't carry them (DEVN-PHASE-06-ERROR-MESSAGE-PRESERVATION). Includes 1 release-discipline Prettier fix-in-milestone commit (`1ecbace`) + R0 direct-fix amendments to PLAN truth predicates after QA surfaced 3 documentation-only fails (`MH-18`: 5 vs predicted 4 commits per Release Discipline §A fix-in-milestone; `ART-08`: 5 missing prescribed SUMMARY body sections added; `DEVN-01`: T3 diff line-count predicate replaced with single-file-modification predicate since the prior hand-rolled parser was already derived from the same grammar — structural equivalence expected; D7 is about commit boundaries, not diff size).

### Process improvements memorialized

- **Clean-PASS pattern (Phase 04 forward):** Pre-registered deviations live in PLAN frontmatter ONLY; SUMMARY `deviations: []`; no body `## Deviations` section. Phases 04, 05, 06 all PASS first-try with this pattern (Phase 06 needed one direct-fix round for documentation predicates — handled per `[feedback_pragmatic_over_protocol]` without full Lead/Dev/QA pipeline).
- **VBW gate quirks documented** in `project_vbw_gate_gitignored_planning_gap` (3 flavors): `paths_cover_required_original_plan_artifacts` on pure plan-amendment rounds (Phase 02 R01 force-advance), `qa_gate_deviation_override=true` on code-fix rounds with SUMMARY deviations (Phase 03 R01 force-advance), `qa_gate_known_issues_override=true` on milestone-wide carry-overs synced into Phase-local registry (Phase 06 — pruned local known-issues.json after QA).

### Carry-overs out of milestone 17

- **DEVN-05** — `packages/dashboard/src/client/components/Popover.tsx:138` TS2322 ARIA-role-union error. Accepted-process-exception across milestones 13/14/15/16/17. Five-milestone carry-over needing a dedicated phase to revisit ARIA-role typing without scope creep.
- **DEVN-PHASE-06-DRIFT-CI-DEFERRED** — CI workflow step running `pnpm gen:apply-patch-parser` + `git diff --exit-code` would close the Lark drift loop end-to-end. Audit script already detects grammar-hash drift via Phase 05's pack-driven refactor; generator regeneration is invoke-able locally. Tracked for follow-up patch.
